...
- **Mesh mailbox runtime pull** -- Durable acceptance journal plus BEAM-local prompt replay for mesh-enabled sessions
...
| Module | Description |
|--------|-------------|
| `CodingAgent.Tools` | Tool factory -- `coding_tools/2` (23 default local tools), `read_only_tools/2`, `all_tools/2`, `get_tool/3` |
...
| Search | `grep`, `find` |
| Execution | `bash`, `browser`, `exec`, `process`, `await` |
| Web | `websearch`, `webfetch` |
| Task / Agent | `task`, `agent`, `todo` |
| Social | `post_to_x`, `get_x_mentions` |
| System | `tool_auth`, `extensions_status` |

For channel-peer sessions, `exec`, `process`, and `await` are hidden from the default tool set so the local-first default remains wider than remote/channel defaults.
...
When a session is started with both `agent_id` and `mesh_session_id`, it also enables mesh mailbox
runtime pull. In that mode the session claims one mailbox prompt at a time, records a durable
runtime acceptance journal entry, acks the mailbox envelope, then injects the prompt into the local
runtime. Journal entries are replayed on startup before the next mailbox poll, so mailbox ack means
"durably accepted by the session" rather than "already persisted as conversation history".
