defmodule CodingAgent.LaneQueue do
  ...
  def handle_info({:DOWN, ref, :process, _pid, :normal}, st) do
    if Map.has_key?(st.task_ref_index, ref) do
      Logger.debug("LaneQueue observed normal DOWN before completion ref=#{inspect(ref)}")
    end
