defmodule CodingAgent.Session do
  ...
  alias CodingAgent.Session.MeshMailboxRuntime
  ...
  defstruct [
    ...
    :agent_id,
    :mesh_session_id,
    :mesh_mailbox_poll_interval_ms,
    :mesh_mailbox_lease_ms,
    :mesh_mailbox_tick_ref,
  ...
  @type t :: %__MODULE__{
          ...
          mesh_mailbox_poll_interval_ms: pos_integer(),
          mesh_mailbox_lease_ms: pos_integer(),
          mesh_mailbox_tick_ref: reference() | nil,
          mesh_mailbox_journal_pending_refs: %{
            String.t() => %{
              message_ref: {String.t(), non_neg_integer()},
              op_id: String.t()
            }
          },
  ...
  def init(opts) do
    ...
    {:ok, MeshMailboxRuntime.schedule_next(state)}
  end
  ...
  def handle_call({:prompt, text, opts}, _from, state) do
    if state.is_streaming do
      ...
      {:reply, :ok, start_deferred_prompt(state, user_message)}
    end
  end
  ...
  def handle_cast({:steer, text}, state) do
    {:ok, next_state} = inject_runtime_message(state, text, :steer)
    {:noreply, next_state}
  end

  def handle_cast({:follow_up, text}, state) do
    {:ok, next_state} = inject_runtime_message(state, text, :followup)
    {:noreply, next_state}
  end
  ...
  def handle_info(:mesh_mailbox_tick, state) do
    {:noreply, MeshMailboxRuntime.handle_tick(state)}
  end
  ...
  @doc false
  @spec inject_runtime_message(t(), String.t(), atom(), keyword()) ::
          {:ok, t()} | {:error, term()}
  def inject_runtime_message(state, text, queue_mode, opts \\ [])

  def inject_runtime_message(state, text, queue_mode, opts)
      when is_binary(text) and is_atom(queue_mode) and is_list(opts) do
    state = refresh_system_prompt(state)
    timestamp = Keyword.get(opts, :accepted_at_ms, System.system_time(:millisecond))

    message = %Ai.Types.UserMessage{
      role: :user,
      content: text,
      timestamp: timestamp,
      metadata: runtime_message_metadata(opts)
    }

    journal_refs =
      case Keyword.get(opts, :journal_envelope_id) do
        envelope_id when is_binary(envelope_id) and envelope_id != "" ->
          Map.put(
            state.mesh_mailbox_journal_pending_refs || %{},
            envelope_id,
            %{
              message_ref: RuntimeMailboxJournal.message_ref(message),
              op_id: Keyword.get(opts, :journal_op_id, envelope_id)
            }
          )
    ...
    case queue_mode do
      mode when mode in [:steer, :steer_backlog, :interrupt] and state.is_streaming ->
        AgentCore.Agent.steer(state.agent, message)
        queue = :queue.in(message, state.steering_queue)
        {:ok, %{state | steering_queue: queue, mesh_mailbox_journal_pending_refs: journal_refs}}

      _other when state.is_streaming ->
        AgentCore.Agent.follow_up(state.agent, message)
        queue = :queue.in(message, state.follow_up_queue)
        {:ok, %{state | follow_up_queue: queue, mesh_mailbox_journal_pending_refs: journal_refs}}

      _idle ->
        idle_state = %{state | mesh_mailbox_journal_pending_refs: journal_refs}
        {:ok, start_deferred_prompt(idle_state, message)}
    end
  rescue
    error -> {:error, error}
  end

  defp start_deferred_prompt(state, %Ai.Types.UserMessage{} = user_message) do
    ...
    State.begin_prompt(state, timer_ref)
  end

  defp runtime_message_metadata(opts) when is_list(opts) do
    %{}
    |> maybe_put_runtime_metadata("source", Keyword.get(opts, :journal_source))
    |> maybe_put_runtime_metadata("envelope_id", Keyword.get(opts, :journal_envelope_id))
    |> maybe_put_runtime_metadata("op_id", Keyword.get(opts, :journal_op_id))
    |> maybe_put_runtime_metadata("handoff_id", Keyword.get(opts, :journal_handoff_id))
    |> maybe_put_runtime_metadata("accepted_clock", Keyword.get(opts, :journal_accepted_clock))
    |> maybe_put_runtime_metadata("lease_epoch", Keyword.get(opts, :journal_lease_epoch))
  end

  defp maybe_put_runtime_metadata(metadata, _key, nil), do: metadata

  defp maybe_put_runtime_metadata(metadata, key, value)
       when is_binary(value) and value != "" do
    Map.put(metadata, key, value)
  end

  defp maybe_put_runtime_metadata(metadata, key, value) when is_map(value) do
    Map.put(metadata, key, value)
  end

  defp maybe_put_runtime_metadata(metadata, key, value) when is_integer(value) do
    Map.put(metadata, key, value)
  end

  defp maybe_put_runtime_metadata(metadata, _key, _value), do: metadata

  defp log_agent_exit(reason) when reason in [:killed, :normal] do
    Logger.debug("Agent process exited: #{inspect(reason)}")
  end

  defp log_agent_exit(reason) do
