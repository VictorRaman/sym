defmodule CodingAgent.ToolExecutor do
  ...
  def execute_with_approval(tool_name, args, execute_fn, context) do
    ...
    with :ok <- validate_cluster_lease(context) do
      case request_approval(
        ...
        other ->
          Logger.warning(
            "Tool #{tool_name} approval returned unexpected value: #{inspect(other)}"
          )
  ...
  defp request_approval(
    ...
    request_fun.(%{
      ...
      agent_id: agent_id,
      node_id: node_id,
      trusted_cluster: context[:trusted_cluster],
      cluster_internal: context[:cluster_internal],
      lease_epoch: context[:lease_epoch],
      worktree_slice_key: context[:worktree_slice_key],
      tool: tool_name,
      action: args,
    ...
    error ->
      if trusted_cluster_context?(context) do
        {:error, {:approval_unavailable, inspect(error)}}
      else
  ...
  defp trusted_cluster_context?(context) when is_map(context) do
    context[:trusted_cluster] == true and context[:cluster_internal] == true
  end

  defp trusted_cluster_context?(_context), do: false

  defp validate_cluster_lease(context) when is_map(context) do
    if trusted_cluster_context?(context) and is_binary(context[:worktree_slice_key]) do
      reader = context[:worktree_lease_reader] || (&LemonMesh.Store.get_worktree_lease/1)

      with {:ok, %WorktreeLease{} = lease} <- reader.(context[:worktree_slice_key]),
           true <- lease.origin_node_id == context[:node_id],
           true <- lease.lease_epoch == context[:lease_epoch],
           true <- WorktreeLease.active?(lease) do
        :ok
      else
        _ ->
          {:error,
  ...
  defp validate_cluster_lease(_context), do: :ok
