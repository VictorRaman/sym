defmodule CodingAgent.ToolPolicy do
  ...
  def subagent_policy(engine, opts \\ []) do
    base_policy = engine_policy_for_subagent(engine, opts)
  ...
  defp engine_policy_for_subagent(:internal, _opts), do: from_profile(:full_access)

  defp engine_policy_for_subagent(engine, opts)
       when engine in [:codex, :claude, :kimi, :opencode, :pi] do
    if local_interactive_opts?(opts) do
      from_profile(:full_access)
    else
      from_profile(:subagent_restricted)
    end
  end

  defp engine_policy_for_subagent(engine, _opts), do: engine_policy(engine)

  defp local_interactive_opts?(opts) do
    case session_key_from_opts(opts) do
      nil ->
        false
  ...
  defp session_key_from_opts(opts) when is_list(opts) do
    Keyword.get(opts, :session_key) || Keyword.get(opts, :parent_session_key)
  end

  defp session_key_from_opts(opts) when is_map(opts) do
    Map.get(opts, :session_key) || Map.get(opts, "session_key") ||
      Map.get(opts, :parent_session_key) || Map.get(opts, "parent_session_key")
  end

  defp session_key_from_opts(_opts), do: nil
