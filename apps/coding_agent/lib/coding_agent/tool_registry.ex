defmodule CodingAgent.ToolRegistry do
  ...
  @local_only_tools MapSet.new(["exec", "process", "await"])
  ...
  @builtin_tools [
    ...
    {:task, Tools.Task},
    {:agent, Tools.Agent},
    {:mesh_mailbox, Tools.MeshMailbox},
  ...
  def get_tools(cwd, opts \\ []) do
    ...
    tools =
      ...
      |> filter_local_only(Keyword.get(opts, :session_key))
  ...
  defp filter_local_only(tools, session_key) when is_binary(session_key) do
    if SessionKey.channel_peer?(session_key) do
      Enum.reject(tools, fn {name, _tool, _source} ->
        MapSet.member?(@local_only_tools, name)
      end)
    else
      tools
    end
  ...
  defp filter_local_only(tools, _session_key), do: tools
