defmodule CodingAgent.Tools.Agent do
      ...
      parameters: %{
        ...
        "properties" => %{
          ...
          "mesh_session_id" => %{
            "type" => "string",
            "description" =>
              "Optional Lemon Mesh session id propagated through delegated request metadata and async followups"
          },
          "queue_mode" => %{
            "type" => "string",
  ...
  defp handle_async_submission(run_id, validated, delegated_session_key, request, opts) do
    task_id =
      TaskStore.new_task(%{
        ...
        mesh_session_id: validated.mesh_session_id
      })
  ...
  defp build_run_request(validated, delegated_session_key, cwd, opts) do
    ...
    with {:ok, prompt} <- maybe_apply_role_prompt(validated.prompt, validated.role_id, role_cwd) do
      ...
      meta =
        ...
        |> maybe_put(:mesh_session_id, validated.mesh_session_id)
        |> compact_map()
      ...
      followup =
        RunRequest.new(%{
          ...
          meta: %{
            ...
            delegated_session_key: delegated_session_key,
            mesh_session_id: Map.get(request.meta || %{}, :mesh_session_id)
          }
        })
  ...
  defp validate_run_params(params, cwd) when is_map(params) do
    ...
    mesh_session_id = Map.get(params, "mesh_session_id") |> normalize_optional_string()
    ...
    cond do
      ...
      not is_nil(mesh_session_id) and not is_binary(mesh_session_id) ->
        {:error, "mesh_session_id must be a string"}
