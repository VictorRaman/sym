defmodule CodingAgent.IntegrationTest do
  ...
  defp wait_for_session_run(session, min_messages \\ 2, timeout \\ 5000) do
    deadline = System.monotonic_time(:millisecond) + timeout
    wait_for_session_run_loop(session, min_messages, deadline)
  end

  defp wait_for_session_run_loop(session, min_messages, deadline) do
    if System.monotonic_time(:millisecond) > deadline do
      :timeout
    else
      state = Session.get_state(session)
