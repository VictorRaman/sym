defmodule CodingAgent.LaneQueueTest do
  ...
  test "error in function returns {:error, _}", %{task_sup: sup} do
    ...
    {result, log} =
      capture_log(fn ->
        ...
        send(self(), {:lane_queue_result, result})
      end)
      |> then(fn log ->
        receive do
          {:lane_queue_result, result} -> {result, log}
        end
      end)

    assert {:error, {%ArgumentError{message: "boom"}, stacktrace}} = result
    assert is_list(stacktrace)
    assert log =~ "LaneQueue job failed"
  end
  ...
  test "successful jobs do not log duplicate completion warnings", %{task_sup: sup} do
    {:ok, pid} =
      LaneQueue.start_link(
        name: :lane_queue_no_duplicate_completion_logs,
        caps: %{main: 1},
        task_supervisor: sup
      )
