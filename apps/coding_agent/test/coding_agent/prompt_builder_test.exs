defmodule CodingAgent.PromptBuilderTest do
  ...
  setup %{tmp_dir: tmp_dir} do
    resolved_tmp_dir =
      case System.cmd("readlink", ["-f", tmp_dir]) do
        {path, 0} -> String.trim(path)
        _ -> tmp_dir
      end

    previous_agent_dir_env = System.get_env("LEMON_AGENT_DIR")
    previous_coding_agent_agent_dir = Application.get_env(:coding_agent, :agent_dir)
    previous_lemon_skills_agent_dir = Application.get_env(:lemon_skills, :agent_dir)

    File.rm_rf!(Path.join(resolved_tmp_dir, ".lemon"))
    File.rm_rf!(Path.join(resolved_tmp_dir, ".agents"))
    File.rm_rf!(Path.join(resolved_tmp_dir, "agent"))

    agent_dir = Path.join(resolved_tmp_dir, "agent")
    File.mkdir_p!(Path.join(agent_dir, "skill"))
    ...
    on_exit(fn ->
      if previous_agent_dir_env do
        System.put_env("LEMON_AGENT_DIR", previous_agent_dir_env)
      else
        System.delete_env("LEMON_AGENT_DIR")
      end

      if is_nil(previous_coding_agent_agent_dir) do
        Application.delete_env(:coding_agent, :agent_dir)
      else
        Application.put_env(:coding_agent, :agent_dir, previous_coding_agent_agent_dir)
      end

      if is_nil(previous_lemon_skills_agent_dir) do
        Application.delete_env(:lemon_skills, :agent_dir)
      else
        Application.put_env(:lemon_skills, :agent_dir, previous_lemon_skills_agent_dir)
      end

      LemonSkills.refresh()
    end)

    {:ok, tmp_dir: resolved_tmp_dir}
  end
