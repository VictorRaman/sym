defmodule CodingAgent.SessionSupervisorComprehensiveTest do
  ...
  describe "session isolation" do
    test "sessions do not share state" do
      ...
      wait_for_session_activity(pid1)

      state1 = Session.get_state(pid1)
      state2 = Session.get_state(pid2)

      # Session 2 should not observe session 1's runtime state or messages.
      assert :queue.len(state1.steering_queue) == 0
      assert :queue.len(state2.steering_queue) == 0
      assert length(Session.get_messages(pid1)) > 0
      assert Session.get_messages(pid2) == []
    end
  ...
  defp wait_for_session_activity(pid, attempts \\ 40)

  defp wait_for_session_activity(pid, attempts) when attempts > 0 do
    if length(Session.get_messages(pid)) > 0 do
      :ok
    else
      Process.sleep(50)
      wait_for_session_activity(pid, attempts - 1)
    end
  end

  defp wait_for_session_activity(_pid, 0), do: flunk("session activity not observed in time")
end
