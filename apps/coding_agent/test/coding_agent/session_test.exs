defmodule CodingAgent.SessionTest do
  ...
  defp wait_for_session_run(session, min_messages \\ 2, attempts \\ 50)

  defp wait_for_session_run(session, min_messages, attempts) when attempts > 0 do
    ...
    cond do
      ...
      true ->
        Process.sleep(10)
        wait_for_session_run(session, min_messages, attempts - 1)
    end
  end

  defp wait_for_session_run(_session, _min_messages, 0), do: flunk("condition not met in time")

  defp wait_for_queue_drain(session, queue_field, attempts \\ 300)

  defp wait_for_queue_drain(session, queue_field, attempts) when attempts > 0 do
    state = fetch_state(session)

    cond do
      not state.is_streaming and :queue.len(Map.fetch!(state, queue_field)) == 0 ->
        :ok

      true ->
        Process.sleep(10)
        wait_for_queue_drain(session, queue_field, attempts - 1)
    end
  end

  defp wait_for_queue_drain(_session, _queue_field, 0),
    do: flunk("queue did not drain in time")

  defp fetch_state(session, timeout \\ 15_000) do
    GenServer.call(session, :get_state, timeout)
  end
  ...
  describe "event broadcasting" do
    test "events are broadcast to subscribers" do
      ...
      assert_receive {:session_event, _session_id, _event}, 10_000
    end
    ...
    test "multiple subscribers receive events" do
      ...
      assert_receive {:session_event, _session_id, _event}, 10_000
      assert_receive {:other_received, _event}, 10_000
    end
  end
  ...
  describe "steer/2" do
    test "idle steering messages start a deferred run" do
      session = start_session()

      :ok = Session.steer(session, "Stop what you're doing")
      wait_for_session_run(session)
    ...
    test "multiple idle steering messages are accepted without leaving queue residue" do
      session = start_session()
  ...
  describe "follow_up/2" do
    test "idle follow-up messages start a deferred run" do
      session = start_session()
    ...
    test "multiple idle follow-up messages are accepted without leaving queue residue" do
      session = start_session()

      :ok = Session.follow_up(session, "First follow-up")
      :ok = Session.follow_up(session, "Second follow-up")
      wait_for_queue_drain(session, :follow_up_queue)

      assert :queue.len(Session.get_state(session).follow_up_queue) == 0
    end
  end
  ...
  describe "reset/1" do
    ...
    test "reset during active run aborts work and stream subscribers get canceled terminal" do
      ...
      :ok = Session.prompt(session, "Hello")
      assert_receive :stream_called, 1_200
  ...
  describe "abort/1" do
    ...
    test "abort cancels deferred prompt before stream starts" do
      ...
      assert_receive :stream_called, 5_000
      wait_for_streaming_complete(session)
    end
  end
  ...
  describe "diagnostics/1" do
    test "returns comprehensive diagnostic info" do
      ...
      assert Map.has_key?(diag, :mesh_session_id)
      assert Map.has_key?(diag, :mesh_mailbox_runtime_enabled)
      assert Map.has_key?(diag, :mesh_mailbox_journal_pending_count)
    end

    test "includes mesh runtime fields when mesh is enabled" do
      Application.ensure_all_started(:lemon_mesh)
      MeshStore.reset()

      {:ok, mesh_pid} = LemonMesh.start_session(goal: "diagnostics mesh")
      mesh_session_id = LemonMesh.session_id(mesh_pid)

      session = start_session(mesh_session_id: mesh_session_id, agent_id: "reviewer")
      diag = Session.diagnostics(session)

      assert diag.mesh_session_id == mesh_session_id
      assert diag.mesh_mailbox_runtime_enabled == true
      assert diag.mesh_mailbox_journal_pending_count == 0
    end
    ...
    test "steering_queue_size stays empty after idle steering is accepted immediately" do
      session = start_session()
      ...
      diag_after = Session.diagnostics(session)
      assert diag_after.steering_queue_size == 0
    end

    test "follow_up_queue_size stays empty after idle follow-up is accepted immediately" do
      session = start_session()
      ...
      diag_after = Session.diagnostics(session)
      assert diag_after.follow_up_queue_size == 0
    end
