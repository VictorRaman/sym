defmodule CodingAgent.Tools.TaskAsyncTest do
  ...
  describe "execute/6 - async auto followup" do
    test "records async task lifecycle in lemon_mesh when mesh_session_id is provided" do
      {:ok, mesh_pid} = LemonMesh.start_session(goal: "Task lifecycle capture")
        ...
        Task.execute(
          ...
          run_override: fn _on_update, _signal ->
            %AgentCore.Types.AgentToolResult{
              content: [%Ai.Types.TextContent{text: "mesh output"}],
      ...
      assert result.details.mesh_session_id == mesh_session_id

      entries = wait_for_blackboard_entries(mesh_session_id, 2)

      assert Enum.any?(entries, fn entry ->
               entry.kind == "task_started" and
                 entry.body["description"] == "Mesh tracked task"
             end)

      assert Enum.any?(entries, fn entry ->
               entry.kind == "task_completed" and
                 entry.body["result_preview"] =~ "mesh output"
             end)
    end
  ...
  defp wait_for_blackboard_entries(mesh_session_id, min_count, attempts \\ 20)

  defp wait_for_blackboard_entries(mesh_session_id, min_count, attempts) when attempts > 0 do
    case LemonMesh.list_blackboard_entries(mesh_session_id) do
      {:ok, entries} when length(entries) >= min_count ->
        entries

      _ ->
        Process.sleep(25)
        wait_for_blackboard_entries(mesh_session_id, min_count, attempts - 1)
    end
  end

  defp wait_for_blackboard_entries(mesh_session_id, _min_count, 0) do
    case LemonMesh.list_blackboard_entries(mesh_session_id) do
      {:ok, entries} -> entries
      _ -> []
    end
