...
Without a token, operators receive the scopes listed in `connect` params (or all operator scopes by default). The server logs a warning for anonymous operator handshakes so exposed deployments surface this risk. With a valid token, role and scopes are derived from the stored identity.
...
Token validation is handled by `LemonControlPlane.Auth.TokenStore` (backed by `LemonCore.Store` under `:session_tokens` namespace). Session tokens are keyed by SHA-256 hash rather than the plaintext token.
...
- The `test_helper.exs` restarts `:lemon_gateway` to reset the absorbed router/channels runtime to a clean baseline; it also disables Telegram
...
| `lemon_gateway` router/channels | `LemonRouter.submit/1`, `LemonRouter.RunOrchestrator`, `LemonRouter.RunRegistry`, and `LemonChannels.Outbox` from absorbed subsystems inside `apps/lemon_gateway/` |
...
| `lemon_gateway` automation | `LemonAutomation.CronManager` for cron CRUD; heartbeat management via absorbed automation modules |
