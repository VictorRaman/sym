...
Without a token, operators receive the scopes listed in `connect` params (or all operator scopes by default). The server now emits a warning for anonymous operator handshakes so exposed deployments surface this risk in logs. With a valid token, role and scopes are derived from the stored identity.
...
Token validation is handled by `Auth.TokenStore`, backed by `LemonCore.Store` under the `:session_tokens` namespace. Session tokens are stored by SHA-256 key rather than plaintext token value, validated on each connection attempt, and expired tokens are cleaned up lazily.
...
| `mesh.agent.message` | write | Send a mesh handoff and report delivery/bookkeeping status |
| `mesh.agent.mailbox.list` | read | List durable mailbox envelopes for a mesh agent |
| `mesh.agent.mailbox.ack` | write | Acknowledge a durable mailbox envelope |
| `mesh.replication.status` | read | Read trusted-peer replication health, membership, watermarks, bootstrap state, target watermark, and backfill progress |
...
The mesh control-plane methods expose a durable single-node collaboration substrate:

- `mesh.agent.message` now persists the handoff, mailbox envelope, and blackboard entry before it
  attempts router fast-path delivery; `deliveryAccepted` reports only that fast-path result
- `mesh.agent.mailbox.list` defaults to pending/unclaimed mail and can expose active leases with
  `includeClaimed=true`
- `mesh.agent.mailbox.ack` supports `expectedClaimedBy` so operators cannot accidentally ack
  another session's lease
- `mesh.replication.status` exposes `bootstrapTargetWatermark`, `backfillComplete`, and
  `backfillLag` per peer so bootstrap/backfill progress is observable instead of inferred
...
| `lemon_gateway` | Run submission via absorbed `LemonRouter` modules and channel delivery via absorbed `LemonChannels` modules |
| `lemon_games` | Games platform match service, auth, rate limiting |
| `lemon_skills` | Skill management |
| `lemon_gateway` automation | Cron manager and heartbeat features via absorbed `LemonAutomation` modules |
