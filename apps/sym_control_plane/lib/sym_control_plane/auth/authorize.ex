defmodule LemonControlPlane.Auth.Authorize do
  ...
  def from_params(params) do
    ...
    case validate_token(token) do
      ...
      _ ->
        ...
        maybe_warn_anonymous_operator(token, role, scopes)
  ...
  defp maybe_warn_anonymous_operator(token, :operator, scopes) when token in [nil, ""] do
    Logger.warning(
      "anonymous operator connection accepted with scopes=#{inspect(scopes)}; configure token auth if this endpoint is exposed"
    )
  end

  defp maybe_warn_anonymous_operator(_token, _role, _scopes), do: :ok
end
