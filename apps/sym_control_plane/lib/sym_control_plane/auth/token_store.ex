defmodule LemonControlPlane.Auth.TokenStore do
  ...
  def store(token, identity, opts \\ []) do
    ...
    Store.put(@store_namespace, store_key, token_info)
    {:ok, token_info}
  end
  ...
  defp fetch_entry(token) when is_binary(token) do
    Store.get(@store_namespace, storage_key(token)) || Store.get(@store_namespace, token)
  end

  defp delete_entry(token) when is_binary(token) do
    Store.delete(@store_namespace, storage_key(token))
