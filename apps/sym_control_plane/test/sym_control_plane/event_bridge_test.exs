defmodule LemonControlPlane.EventBridgeTest do
  ...
  setup do
    ...
    conn_id = "test-conn-#{System.unique_integer([:positive])}"
    :ok = Presence.register(conn_id, %{role: :operator, client_id: "test-client", pid: self()})
  ...
  describe "event forwarding" do
    ...
    test "redacts sensitive tool arguments before broadcasting" do
      run_id = "run_#{System.unique_integer()}"
      ...
      event =
        LemonCore.Event.new(
          ...
          %{
            ...
            title: "Bash",
            detail: %{
              "args" => %{"token" => "topsecret", "safe" => "ok"},
              "result" => %{"authorization" => "Bearer abc", "safe" => "done"}
            },
            phase: :completed,
            ok: true
          },
      ...
      assert_receive {:event, "agent", payload, _state_version}, 1000
      assert get_in(payload, ["action", "detail", "args", "token"]) == "[REDACTED]"
      assert get_in(payload, ["action", "detail", "args", "safe"]) == "ok"
      assert get_in(payload, ["action", "detail", "result", "authorization"]) == "[REDACTED]"
  ...
  describe "static topic subscriptions" do
    ...
    test "redacts pairing codes from node pairing events" do
      if Code.ensure_loaded?(LemonCore.Bus) do
        event =
          LemonCore.Event.new(
            :node_pair_requested,
        ...
        assert_receive {:event, "node.pair.requested", payload, _state_version}, 1000
