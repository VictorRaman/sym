defmodule LemonControlPlane.Methods.ConnectChallengeTest do
  ...
  describe "device pairing flow with connect.challenge" do
    test "full device pairing flow produces valid challenge token" do
      ...
      assert LemonCore.Store.get(:device_pairing_challenges, challenge_token) == nil
      assert LemonCore.Store.get(:devices, approve_result["deviceToken"]) == nil
    end
  ...
  describe "node pairing flow with connect.challenge" do
    test "full node pairing flow produces valid challenge token" do
      ...
      assert LemonCore.Store.get(:node_challenges, challenge_token) == nil
    end
  ...
  describe "pairing request persistence" do
    test "device pairing requests do not persist the plaintext pairing code" do
      {:ok, request_result} =
      ...
      stored = LemonCore.Store.get(:device_pairing, request_result["pairingId"])
      refute Map.has_key?(stored, :code)
      refute Map.has_key?(stored, "code")
    end

    test "node pairing requests do not persist the plaintext pairing code" do
      {:ok, request_result} =
        NodePairRequest.handle(
          %{
            "nodeType" => "compute",
            "nodeName" => "Stored Worker"
          },
          @pairing_ctx
        )

      stored = LemonCore.Store.get(:nodes_pairing, request_result["pairingId"])
      refute Map.has_key?(stored, :code)
      refute Map.has_key?(stored, "code")
      assert LemonCore.Store.get(:nodes_pairing_by_code, request_result["code"]) == nil
    end
  end
