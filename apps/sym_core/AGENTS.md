...
| `LemonCore.RouterBridge` | Runtime bridge to the absorbed `LemonRouter` namespace inside `:lemon_gateway` without compile-time coupling |
...
```bash
# Cleanup old run artifacts plus Lemon repo/tmp residue
mix lemon.cleanup
mix lemon.cleanup --apply --retention-days 30

# Also delete local build artifacts (_build, _codex_build, deps, native/*/target, clients/**/dist, node_modules)
mix lemon.cleanup --apply --build-artifacts --retention-days 30
...
Channel adapters and other producers forward runs to the absorbed `LemonRouter` namespace without a compile-time dependency. The router runtime registers itself at startup from `:lemon_gateway`.
...
- **LemonRouter namespace** -- Registers via `RouterBridge.configure/1` at startup from `:lemon_gateway`; uses `SessionKey`, `Store`, `Bus`, `Config`
- **lemon_gateway** -- Uses `GatewayConfig`, `Config`, `Bus`, `Store` for gateway lifecycle
- **LemonChannels subsystem** -- Lives inside `:lemon_gateway`; uses `InboundMessage`, `RouterBridge`, `Bus`, `Binding`, `SessionKey`
- **coding_agent** -- Uses `Config` (provider/model resolution), `Secrets` (API key resolution), `Store`, `Bus`
- **LemonAutomation subsystem** -- Lives inside `:lemon_gateway`; uses `Store` (cron jobs), `Bus` (cron events), `Config`
...
- `LemonCore.RouterBridge` returns `{:error, :unavailable}` when the absorbed `LemonRouter` runtime has not registered itself; callers must handle this gracefully
