...
+---------+--------+
| LemonCore.       |
| RouterBridge     |  Runtime bridge to the
| SessionKey       |  absorbed `LemonRouter`
| RunRequest       |  namespace in `:lemon_gateway`
| InboundMessage   |  without compile-time coupling
+------------------+
```
...
| `mix lemon.cleanup` | Clean up old run artifacts plus Lemon repo/tmp residue |
...
Tool execution gating with scope-based persistence. Placed in `lemon_core` so any app can request/resolve approvals without depending on the absorbed `LemonRouter` runtime namespace inside `:lemon_gateway`.
...
Channel adapters forward runs to the absorbed `LemonRouter` namespace without compile-time coupling. The router registers itself at startup from `:lemon_gateway`.
...
Returns `{:error, :unavailable}` when the absorbed `LemonRouter` runtime has not registered; callers must handle this gracefully.
...
- `RouterBridge` returns `{:error, :unavailable}` when the absorbed `LemonRouter` runtime has not registered
