defmodule LemonCore.Quality.ArchitectureRulesCheck do
  ...
  @rules [
    %{
      ...
      files: ["apps/lemon_gateway/lib/lemon_router/**/*.ex"],
    ...
    %{
      ...
      exclude: ["apps/lemon_gateway/lib/lemon_router/agent_directory.ex"]
    },
    %{
    ...
    %{
      ...
      exclude: [
        "apps/lemon_gateway/lib/lemon_router/**/*.ex",
        "apps/lemon_gateway/test/lemon_router/**/*.exs",
    ...
    %{
      ...
      files: [
        ...
        "apps/lemon_gateway/lib/lemon_router/agent_directory.ex"
      ],
      patterns: [
    ...
    %{
      ...
      files: [
        "apps/lemon_gateway/lib/lemon_channels/**/*.ex",
        "apps/lemon_gateway/lib/lemon_router/**/*.ex"
      ],
      exclude: [
        "apps/lemon_gateway/lib/lemon_channels/telegram/known_target_store.ex",
        "apps/lemon_core/lib/lemon_core/store.ex"
      ],
      patterns: [
    ...
    %{
      ...
      exclude: ["apps/lemon_gateway/lib/lemon_router/**/*.ex"],
      patterns: [
    ...
    %{
      ...
      files: [
        "apps/lemon_gateway/lib/lemon_automation/**/*.ex",
    ...
    %{
      ...
      files: ["apps/lemon_gateway/lib/lemon_router/**/*.ex", "apps/lemon_control_plane/lib/**/*.ex"],
      exclude: [
        "apps/lemon_gateway/lib/lemon_router/agent_endpoint_store.ex",
        "apps/lemon_core/lib/lemon_core/store.ex"
      ],
      patterns: [
  ...
  def run(opts \\ []) do
    ...
    issues =
      @rules
      |> Enum.flat_map(&rule_issues(root, &1, all_files))
      |> Enum.sort_by(& &1.path)
  ...
  defp rule_issues(root, rule, all_files) do
    rule.files
    |> source_files(all_files)
    |> reject_excluded(Map.get(rule, :exclude, []), all_files)
  ...
  defp source_files(globs, all_files) do
    globs
    |> Enum.flat_map(fn glob ->
      regex = glob_to_regex(glob)
      Enum.filter(all_files, &Regex.match?(regex, &1))
    end)
    |> Enum.uniq()
  end

  defp reject_excluded(files, [], _all_files), do: files

  defp reject_excluded(files, globs, all_files) do
    excluded =
      globs
      |> source_files(all_files)
      |> MapSet.new()
  ...
  defp all_source_files(root) do
    case System.find_executable("rg") do
      ...
      rg_path ->
        case System.cmd(rg_path, ["--files", "apps"], cd: root, stderr_to_stdout: true) do
          {output, 0} ->
            output
            |> String.split("\n", trim: true)
    ...
    case File.ls(path) do
      {:ok, entries} ->
        Enum.flat_map(entries, fn entry ->
          ...
          cond do
            File.dir?(full_path) ->
              walk_source_files(full_path)

            Path.extname(full_path) in [".ex", ".exs"] ->
              [full_path]
  ...
  defp glob_to_regex(glob) do
    glob
    |> String.replace("/**/", "/__DOUBLE_STAR_DIR__")
    |> Regex.escape()
    |> String.replace("__DOUBLE_STAR_DIR__", "(?:.*/)?")
    |> String.replace("\\*\\*", ".*")
    |> String.replace("\\*", "[^/]*")
