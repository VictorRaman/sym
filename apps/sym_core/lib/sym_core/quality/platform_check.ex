defmodule LemonCore.Quality.PlatformCheck do
  ...
  defp check_profile_policy(issues, manifest) do
    Enum.reduce(manifest, issues, fn {app, entry}, acc ->
      acc
      |> maybe_issue(
        not (entry.tier == :runtime_core and entry.profiles != [:core, :platform]),
        :invalid_platform_profile,
        "Runtime core apps must be present in core and platform profiles",
        app
      )
      |> maybe_issue(
        not (entry.tier == :platform_runtime and entry.profiles != [:platform]),
        :invalid_platform_profile,
        "Platform runtime apps must be platform-profile only",
        app
      )
    end)
  end
