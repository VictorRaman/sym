defmodule Mix.Tasks.Lemon.Onboard.AntigravityTest do
  ...
  alias Mix.Tasks.Lemon.Onboard.Antigravity

  setup do
    tmp_dir =
      Path.join(
        System.tmp_dir!(),
        "lemon_onboard_antigravity_#{System.unique_integer([:positive])}"
      )
  ...
  test "stores token and updates providers config", %{tmp_dir: tmp_dir} do
    ...
    output =
      capture_io(fn ->
        Antigravity.run([
          "--token",
          "antigravity-token-123",
          "--config-path",
          config_path
        ])
      end)

    assert output =~ "Google Antigravity onboarding complete."

    assert {:ok, "antigravity-token-123"} =
             Secrets.get("llm_google_antigravity_api_key", prefer_env: false, env_fallback: false)

    {:ok, config_map} = TomlCompat.decode_file(config_path)

    assert get_in(config_map, ["providers", "google_antigravity", "api_key_secret"]) ==
  ...
  test "sets defaults when --set-default is enabled", %{tmp_dir: tmp_dir} do
    ...
    assert get_in(config_map, ["defaults", "provider"]) == "google_antigravity"

    assert get_in(config_map, ["defaults", "model"]) ==
             "google_antigravity:gemini-3-pro-high"
  end
