defmodule LemonCore.Quality.ArchitectureCheckTest do
  ...
  describe "allowed_direct_deps/0" do
    ...
    test "all known apps have dependency policies" do
      ...
      assert :coding_agent in known_apps
      assert :lemon_gateway in known_apps
      assert :lemon_control_plane in known_apps
      refute :agent_core in known_apps
      refute :lemon_router in known_apps
      refute :lemon_channels in known_apps
    end
  ...
  describe "namespace violation detection" do
    test "attributes CodingAgent.UI.RPC ownership to coding_agent" do
      tmp_dir = create_tmp_umbrella()
      ...
      try do
        assert {status, report} = ArchitectureCheck.run(root: tmp_dir)
        assert status in [:ok, :error]
        refute Enum.any?(report.issues, &(&1.code == :forbidden_namespace_reference))
      after
        File.rm_rf!(tmp_dir)
      end
    end
