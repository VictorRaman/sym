defmodule LemonCore.Quality.DocsCheckTest do
  ...
  describe "absorbed app path references" do
    test "detects stale umbrella app path references in active docs" do
      tmp_dir = create_tmp_dir()
      File.mkdir_p!(Path.join(tmp_dir, "apps/lemon_gateway"))

      File.write!(
        Path.join(tmp_dir, "docs/README.md"),
        """
        ## References

        Refs: `apps/lemon_mesh/`, `apps/lemon_gateway/`
        """
      )

      write_catalog!(tmp_dir, ["docs/README.md"])

      try do
        assert {:error, report} = DocsCheck.run(root: tmp_dir)

        assert Enum.any?(report.issues, fn issue ->
                 issue.code == :missing_app_path_reference and
                   issue.path == "docs/README.md" and
                   issue.message =~ "apps/lemon_mesh/"
               end)
      after
        File.rm_rf!(tmp_dir)
      end
    ...
    test "allows active docs to reference existing absorbed implementation paths" do
      tmp_dir = create_tmp_dir()
      File.mkdir_p!(Path.join(tmp_dir, "apps/lemon_gateway/lib/lemon_router"))
      ...
      File.write!(
        ...
        Refs: `apps/lemon_gateway/lib/lemon_router/`
        """
      )
    ...
    test "detects active docs that describe absorbed subsystems as standalone apps" do
      tmp_dir = create_tmp_dir()
      ...
      File.write!(
        ...
        Telegram is handled by the `lemon_channels` external app.
        """
      )
      ...
      try do
        ...
        assert Enum.any?(report.issues, fn issue ->
                 issue.code == :stale_absorbed_subsystem_narrative and
                   issue.path == "docs/README.md" and
                   issue.message =~ "lemon_channels"
               end)
      after
        File.rm_rf!(tmp_dir)
      end
    ...
    test "allows active docs to describe absorbed subsystems explicitly as subsystems" do
      tmp_dir = create_tmp_dir()
      File.mkdir_p!(Path.join(tmp_dir, "apps/lemon_gateway/lib/lemon_channels"))
      ...
      File.write!(
        ...
        Telegram is handled by the absorbed `LemonChannels` subsystem under `apps/lemon_gateway/lib/lemon_channels/`.
        """
      )
  ...
  defp write_catalog!(root, paths) do
    entries =
      paths
      |> Enum.map(fn path ->
        "%{path: #{inspect(path)}, owner: \"test\", last_reviewed: Date.from_iso8601!(#{inspect(Date.to_iso8601(Date.utc_today()))}), max_age_days: 365}"
      end)
      |> Enum.join(",\n  ")

    File.write!(Path.join(root, "docs/catalog.exs"), "[\n  #{entries}\n]\n")
  end
end
