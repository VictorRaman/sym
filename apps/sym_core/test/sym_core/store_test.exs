defmodule LemonCore.StoreTest do
  ...
  describe "generic update/3" do
    test "updates a single key atomically and returns caller reply" do
      token = unique_token()
      key = {:store_update, token}

      assert :ok = Store.put(:sessions_index, key, %{count: 1})

      assert {:ok, 2} =
               Store.update(:sessions_index, key, fn current ->
                 next = %{current | count: current.count + 1}
                 {:ok, next, {:ok, next.count}}
               end)

      assert Store.get(:sessions_index, key) == %{count: 2}
    end

    test "can delete a key from inside update/3" do
      token = unique_token()
      key = {:store_update_delete, token}

      assert :ok = Store.put(:sessions_index, key, %{count: 1})

      assert :deleted =
               Store.update(:sessions_index, key, fn _current ->
                 {:delete, :deleted}
               end)
  ...
  describe "introspection event storage contract" do
    test "append/list stores canonical events newest-first with filters and limit" do
      ...
      wait_for_introspection_events(run_id, 3)
  ...
  defp wait_for_introspection_events(run_id, expected_count, timeout_ms \\ 5_000) do
    deadline = System.monotonic_time(:millisecond) + timeout_ms
    do_wait_for_introspection_events(run_id, expected_count, deadline)
  end

  defp do_wait_for_introspection_events(run_id, expected_count, deadline_ms) do
    count =
      Store.list_introspection_events(run_id: run_id, limit: expected_count)
      |> length()
    ...
    if count >= expected_count do
      ...
      if System.monotonic_time(:millisecond) >= deadline_ms do
        flunk("introspection events were not persisted in time")
      else
        Process.sleep(10)
        do_wait_for_introspection_events(run_id, expected_count, deadline_ms)
      end
    end
  end
