...
+-----------------------------------------------------------------------+
|                           Transports                                   |
|  Telegram (LemonChannels)  Discord (LemonChannels)  XMTP (LemonChannels) |
...
| `LemonChannels` subsystem | Absorbed into `:lemon_gateway`. Telegram/Discord/XMTP adapters live under `apps/lemon_gateway/lib/lemon_channels/` and consume gateway bus events. |

### Dependents (apps that depend on this)

| App | How It Uses LemonGateway |
|-----|-------------------------|
| `LemonRouter` namespace | Subscribes to `LemonCore.Bus` events from Run processes and manages `RunProcess` plus `StreamCoalescer` from inside `:lemon_gateway` |
| `LemonChannels` subsystem | Subscribes to bus events for channel-specific rendering and delivery from inside `:lemon_gateway` |
| `lemon_control_plane` | May submit jobs or cancel runs via `Runtime` API |
| `LemonAutomation` subsystem | Submits jobs for scheduled/cron runs from inside `:lemon_gateway` |
...
- `LemonChannels` -- Telegram/Discord/XMTP adapters in the absorbed subsystem under `apps/lemon_gateway/lib/lemon_channels/`
