defmodule LemonGateway.Application do
  ...
  def start(_type, _args) do
    children =
      [
        absorbed_child_spec(LemonChannels.Application),
        absorbed_child_spec(LemonRouter.Application),
        absorbed_child_spec(LemonAutomation.Application),
        LemonGateway.Config,
  ...
  defp health_server_child_spec do
    ...
    startup_log = Application.get_env(:lemon_gateway, :health_startup_log, default_startup_log())
    ...
    %{
      ...
      start:
        {Bandit, :start_link,
         [
           [
             plug: LemonGateway.Health.Router,
             ip: ip,
             port: port,
             scheme: :http,
  ...
  defp default_startup_log do
    if test_env?(), do: false, else: :info
  end

  defp absorbed_child_spec(module) when is_atom(module) do
    %{
      id: module,
      start: {module, :start, [:normal, []]},
      type: :supervisor
    }
  end
end
