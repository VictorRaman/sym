defmodule LemonRouter do
  @moduledoc """
  Compatibility facade for the absorbed router subsystem inside `:lemon_gateway`.

  `LemonRouter.*` remains the public module namespace for routing/session
  semantics, but it is no longer a standalone umbrella app. The implementation
  lives under `apps/lemon_gateway/lib/lemon_router/`.
