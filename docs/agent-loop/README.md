...
- Run periodic entropy cleanup for run artifacts:
  ...
  - `mix lemon.cleanup --apply --retention-days 21` (prune old run files plus Lemon repo/tmp residue)
  - `mix lemon.cleanup --apply --build-artifacts --retention-days 21` (also delete local build artifacts such as `_build`, `deps`, `native/*/target`, and client `dist` output)
