...
- `LEMON_MESH_TRUSTED_PEERS` as a comma-separated list
- `LEMON_MESH_REPLICATION_POLL_INTERVAL_MS`
- `LEMON_MESH_REPLICATION_BATCH_LIMIT`
- `LEMON_MESH_SNAPSHOT_SCOPE`
- `LEMON_MESH_LEASE_TTL_MS`
...
Preflight it locally before running the smoke:

```bash
./scripts/control_plane_codex_smoke.mjs --check-only
```

This smoke targets the Codex CLI engine path. It does not require
