...
| Reduced-platform provider secret refs | Secret names configured in provider/runtime config | `LemonCore.Secrets.resolve/2` via runtime-specific secret resolution | Store first, optional env fallback | `apps/lemon_core/lib/lemon_core/secrets.ex` |
