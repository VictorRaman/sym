...
Lemon is a platform umbrella with explicit product grading:
...
Use [`docs/platform_tiers.md`](/mnt/e/lemon-main/docs/platform_tiers.md) as the
canonical source for runtime profile membership, maturity status, and keep
rationale. Dependency legality still lives in
...
For targeted regressions or any environment where multiple `mix` commands may run in parallel,
prefer the isolated wrapper so each run gets a unique build/cache root and a version preflight:
...
*Last updated: 2026-03-25* (added platform tiers, runtime profile grading, and incubator/sidecar status guidance)
