...
The easiest way to use lemon day-to-day is through **Telegram**: you talk to a bot from your phone/desktop, while Lemon runs locally. Inbound/outbound messaging is handled by `lemon_channels` (Telegram adapter + outbox), runs are orchestrated by `lemon_router`, and execution is handled by `lemon_gateway` using one of the configured engines (native Lemon, Claude CLI, Codex CLI, OpenCode CLI, Pi CLI). Kimi is also available as a CodingAgent `task` subagent engine.

If you're here for the architecture deep-dive, jump to [What is Lemon?](#what-is-lemon).

Lemon is organized as a multi-product umbrella, not a single lightweight app.
The platform now distinguishes between runtime core, platform runtime,
default-start product surfaces, and incubator/sidecar apps. See
[`docs/platform_tiers.md`](docs/platform_tiers.md) for the canonical grading.
...
Lemon keeps all products in one umbrella, but they are not equal in criticality:

- `Runtime Core`: `lemon_core`, `ai`, `agent_core`, `lemon_mesh`, `lemon_skills`, `coding_agent`
- `Platform Runtime`: `lemon_gateway`, `lemon_router`, `lemon_channels`, `lemon_control_plane`, `lemon_automation`
- `Default-Start Product Surfaces`: `lemon_web`, `lemon_games`, `lemon_sim`, `lemon_sim_ui`
- `Incubator / Sidecar`: `coding_agent_ui`, `lemon_mcp`, `lemon_services`, `market_intel`

`./bin/lemon` still defaults to the current full bundle for compatibility, but
runtime profile membership and app status are now tracked explicitly in
...
5. **Multi-Provider Abstraction**: Unified interface for 26 providers (Anthropic, OpenAI, OpenAI Codex, Google, Google Vertex, Google Gemini CLI, Google Antigravity, Azure OpenAI, AWS Bedrock, xAI, Mistral, Cerebras, DeepSeek, Qwen, MiniMax, MiniMax CN, Z.ai, Kimi, Kimi Coding, OpenCode, OpenCode Go, GitHub Copilot, Groq, HuggingFace, OpenRouter, Vercel AI Gateway) with automatic model configuration and cost tracking.
...
**Control & Routing:**
- **Control plane server** with 112+ RPC methods over WebSocket
...
- **Control Plane**: LemonControlPlane provides HTTP/WebSocket server with 112+ RPC methods
...
│   ├── lemon-xmtp-bootstrap    # XMTP bootstrap helper
│   ├── install-security-hook   # Security pre-push hook installer
...
│   │   └── lib/ai/
│   │       ├── providers/       # 26 providers: Anthropic, OpenAI, Google, Azure, Bedrock, xAI, Mistral, Cerebras, DeepSeek, Qwen, MiniMax, Z.ai, and more
...
│   │   └── lib/lemon_sim/
│   │       ├── state.ex         # World state + event window + intent
│   │       ├── projector.ex     # Decision context projection behaviour
│   │       ├── updater.ex       # Event -> state update behaviour
...
│   ├── setup_games_cron_job.exs # Games cron setup
│   └── x_*.exs, x_get_tokens.sh  # X/Twitter API helpers
│
├── docs/                        # Documentation
...
**Supported Providers (26 total, 200+ models):**
...
- **112+ RPC Methods**: Comprehensive API for all Lemon operations
...
```bash
# Start the default full runtime profile
./bin/lemon

# Start only the platform profile (core + routing + control plane runtime)
./bin/lemon --profile platform
...
`./bin/lemon` defaults to the `full` profile for compatibility. Set
`LEMON_RUNTIME_PROFILE` or pass `--profile <full|platform|core>` to select a
smaller startup surface.
...
For repeatable targeted verification, prefer the isolated wrapper instead of reusing one shared
`_build/` / rebar cache across concurrent shells:

```bash
# Per-app targeted regression tests with isolated build/cache roots
...
# Full compile / quality gates from the umbrella root
scripts/mix_isolated.sh -- compile --warnings-as-errors
scripts/mix_isolated.sh -- lemon.quality

# Pin a specific local toolchain when needed
scripts/mix_isolated.sh --toolchain-bin "$HOME/.elixir-install/installs/elixir/1.19.5-otp-27/bin" -- compile
```

`scripts/mix_isolated.sh` also fails fast when the installed Elixir toolchain is below the
minimum required by the umbrella apps (`1.19+`), and it will auto-select a newer local
`~/.elixir-install` toolchain before failing if your default shell still points at an older
Elixir.
...
# Optional: include the slower live Codex code-task coverage
LEMON_CODEX_LIVE_CODE_TASK=1 \
LEMON_CODEX_INTEGRATION=1 \
mix test apps/agent_core/test/agent_core/cli_runners/codex_integration_test.exs --include integration
```
...
- **`lemon_control_plane`**: HTTP/WebSocket server with 112+ RPC methods
- **`lemon_games`**: Turn-based games platform (RPS/Connect4) with event-sourced matches
- **`lemon_sim`**: Reusable simulation harness contracts and runner primitives
...
| [mesh_runtime.md](docs/mesh_runtime.md) | Lemon Mesh blackboard, peer mailbox leases, runtime pull, and known delivery limits |
