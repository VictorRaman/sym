...
`test/agent_core/cli_runners/codex_integration_test.exs` should run against a hermetic Codex home:

- isolate `HOME`, `CODEX_HOME`, and `XDG_CACHE_HOME`
- copy only the minimum auth material needed to log in
- do not point live integration at the operator's real `~/.codex/memories` or `~/.codex/skills`

The goal is to prove runner correctness without accidentally depending on host-side memories or
skill caches.
