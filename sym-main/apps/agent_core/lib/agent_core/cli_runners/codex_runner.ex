defmodule AgentCore.CliRunners.CodexRunner do
  ...
  defmodule RunnerState do
    ...
    def new(config \\ nil, model_override \\ nil, session_key \\ nil) do
      %__MODULE__{
  ...
  def init_state(_prompt, _resume, cwd) do
    ...
    %{RunnerState.new(LemonConfig.load(cwd)) | cwd: cwd}
  end
  ...
  def init_state(_prompt, _resume, cwd, opts) do
    ...
    %{
      RunnerState.new(LemonConfig.load(cwd), model_override, Keyword.get(opts, :session_key))
      | cwd: cwd,
  ...
  def translate_event(data, state) do
    case data do
      ...
      %ThreadStarted{thread_id: thread_id} ->
        ...
        {event, factory} =
          EventFactory.started(
            state.factory,
            token,
            title: "Codex",
            meta: started_meta(state, thread_id)
          )
  ...
  def handle_exit_error(exit_code, state) do
    Introspection.record(
      ...
      %{
        ...
        session_key: state.session_key,
        cwd: state.cwd,
        thread_id: state.found_session && state.found_session.value
      },
      engine: @engine,
      provenance: :inferred
    )
    ...
    detail = terminal_detail(state, :exit_error, %{exit_code: exit_code})
    log_terminal_diagnostic(message, detail)
    {note_event, factory} = terminal_warning(state.factory, "codex.exit_error", message, detail)
  ...
  def handle_stream_end(state) do
    {reason, message} =
      if state.found_session == nil do
        {:missing_session, "codex exec finished but no session_id/thread_id was captured"}
      else
        {:missing_turn_completion, "codex exec ended without turn completion"}
      end

    detail = terminal_detail(state, reason)
    log_terminal_diagnostic(message, detail)
    {note_event, factory} = terminal_warning(state.factory, "codex.stream_end", message, detail)
  ...
  defp started_meta(state, thread_id) do
    %{
      thread_id: thread_id,
      run_id: state.run_id,
      session_key: state.session_key,
  ...
  defp terminal_warning(factory, action_id, title, detail) do
    EventFactory.action_completed(factory, action_id, :warning, title, false,
      detail: detail,
      message: title,
      level: :warning
    )
  ...
  defp terminal_detail(state, reason, extra \\ %{}) do
    Map.merge(
      %{
        reason: reason,
        thread_id: state.found_session && state.found_session.value,
        ...
        session_key: state.session_key,
        cwd: state.cwd,
        model: codex_model(state),
        has_partial_answer: is_binary(state.final_answer) and state.final_answer != ""
      },
      extra
    )
  end

  defp log_terminal_diagnostic(message, detail) do
    Logger.warning(
      "#{message} thread_id=#{inspect(detail.thread_id)} run_id=#{inspect(detail.run_id)} " <>
        "session_key=#{inspect(detail.session_key)} cwd=#{inspect(detail.cwd)} " <>
        "reason=#{inspect(detail.reason)}"
    )
  end
  ...
  defp codex_auto_approve?(state) do
    case get_codex_config(state, :auto_approve, nil) do
      nil ->
        local_main_session?(state.session_key)
  ...
  defp local_main_session?(session_key) when is_binary(session_key) do
    not SessionKey.channel_peer?(session_key)
  end

  defp local_main_session?(_session_key), do: false
  ...
  defp get_codex_config(%RunnerState{config: %LemonConfig{agent: agent}}, key, default) do
    case env_codex_config(key, :__missing__) do
      :__missing__ ->
  ...
  defp get_codex_config(_state, key, default) do
    case env_codex_config(key, :__missing__) do
      :__missing__ -> default
      value -> value
    end
  ...
  defp env_codex_config(:auto_approve, default) do
  ...
  defp env_codex_config(:extra_args, default) do
  ...
  defp env_codex_config(_key, default), do: default
