defmodule AgentCore.CliRunners.CodexIntegrationTest do
  ...
  setup do
    case System.find_executable("codex") do
      ...
      _path ->
        ...
        case build_hermetic_env(tmp_dir) do
          {:ok, env} ->
            on_exit(fn ->
              File.rm_rf!(tmp_dir)
            end)

            {:ok, cwd: tmp_dir, env: env}
  ...
  describe "basic execution" do
    @tag timeout: 60_000
    test "starts a session and receives events", %{cwd: cwd, env: env} do
      {:ok, session} =
        CodexSubagent.start(
  ...
  describe "slow live code tasks" do
    @tag timeout: 180_000
    test "handles simple code task", %{cwd: cwd, env: env} do
      if System.get_env("LEMON_CODEX_LIVE_CODE_TASK") == "1" do
        {:ok, session} =
          CodexSubagent.start(
        ...
        assert File.exists?(Path.join(cwd, "hello.txt"))
        assert File.read!(Path.join(cwd, "hello.txt")) == "Hello, World!"
      else
        IO.puts("\n=== Code Task Test Skipped ===")

        IO.puts(
          "  Set LEMON_CODEX_LIVE_CODE_TASK=1 to include slower live Codex file-edit coverage."
        )
  ...
  describe "session continuation" do
    @tag timeout: 120_000
    test "can continue a session with follow-up prompt", %{cwd: cwd, env: env} do
      # First prompt
      {:ok, session1} =
        CodexSubagent.start(
      ...
      {:ok, session2} =
        CodexSubagent.continue(session1, "What number did I ask you to remember?", env: env)
    ...
    @tag timeout: 120_000
    test "can resume session using saved token", %{cwd: cwd, env: env} do
  ...
  describe "action events" do
    @tag timeout: 60_000
    test "receives action events for commands", %{cwd: cwd, env: env} do
      {:ok, session} =
        CodexSubagent.start(
  ...
  describe "minimal input" do
    @tag timeout: 60_000
    test "handles minimal prompt and returns output", %{cwd: cwd, env: env} do
      {:ok, session} =
        CodexSubagent.start(
  ...
  defp build_hermetic_env(tmp_dir) do
    host_auth = Path.join(System.user_home!(), ".codex/auth.json")

    if File.exists?(host_auth) do
      ...
      {:skip, "codex auth.json not available for hermetic live integration"}
    end
  end
end
