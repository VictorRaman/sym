defmodule AgentCore.CliRunners.CodexRunnerTest do
  ...
  alias Elixir.AgentCore.CliRunners.CodexRunner
  alias Elixir.AgentCore.CliRunners.CodexRunner.RunnerState

  alias Elixir.AgentCore.CliRunners.CodexSchema.{
    AgentMessageItem,
  ...
  alias Elixir.AgentCore.CliRunners.CodexSchema.ThreadError
  ...
  describe "build_command/3" do
    ...
    test "defaults auto-approve on for local main sessions" do
      state = RunnerState.new(nil, nil, "agent:test:main")
      {_cmd, args} = CodexRunner.build_command("Hello", nil, state)

      assert "--dangerously-bypass-approvals-and-sandbox" in args
    end

    test "does not default auto-approve on for channel peer sessions" do
      state = RunnerState.new(nil, nil, "agent:test:telegram:default:dm:peer-123")
      {_cmd, args} = CodexRunner.build_command("Hello", nil, state)
  ...
  describe "init_state/2" do
    ...
    test "captures cwd, run_id, and session_key from init_state/4 options" do
      state =
        CodexRunner.init_state(
          "Prompt 1",
          nil,
          "/tmp/codex-runner",
          run_id: "run_codex_123",
          session_key: "agent:reviewer:main"
        )

      assert state.cwd == "/tmp/codex-runner"
      assert state.run_id == "run_codex_123"
      assert state.session_key == "agent:reviewer:main"
    end
  end
  ...
  describe "translate_event/2 - session lifecycle" do
    test "translates ThreadStarted to StartedEvent" do
      ...
      assert started.meta == %{thread_id: "thread_abc"}
      assert new_state.found_session.value == "thread_abc"
      assert opts[:found_session].value == "thread_abc"
    end

    test "includes diagnostic meta on ThreadStarted when context is present" do
      state = %{
        RunnerState.new(nil, nil, "agent:reviewer:main")
        | cwd: "/tmp/project",
          run_id: "run_codex_123"
      }

      event = %ThreadStarted{thread_id: "thread_diag"}

      {events, _new_state, _opts} = CodexRunner.translate_event(event, state)

      assert [%StartedEvent{} = started] = events
      assert started.meta.thread_id == "thread_diag"
      assert started.meta.run_id == "run_codex_123"
      assert started.meta.session_key == "agent:reviewer:main"
      assert started.meta.cwd == "/tmp/project"
    end
  ...
  describe "handle_exit_error/2" do
    test "returns note and completed error events" do
      ...
      assert note.action.detail.exit_code == 1
      assert note.action.detail.thread_id == "t_1"
      assert note.action.detail.reason == :exit_error
      assert completed.ok == false
      assert completed.error =~ "rc=1"
    end

    test "includes diagnostic context in exit-error warning details" do
      state = %{
        RunnerState.new(nil, nil, "agent:reviewer:main")
        | final_answer: "partial",
          found_session: ResumeToken.new("codex", "thread_abc"),
          cwd: "/tmp/codex-project",
          run_id: "run_codex_456"
      }

      {[note, _completed], _new_state} = CodexRunner.handle_exit_error(127, state)

      assert note.action.detail.exit_code == 127
      assert note.action.detail.thread_id == "thread_abc"
      assert note.action.detail.run_id == "run_codex_456"
      assert note.action.detail.session_key == "agent:reviewer:main"
      assert note.action.detail.cwd == "/tmp/codex-project"
      assert note.action.detail.has_partial_answer == true
    end
  ...
  describe "handle_stream_end/1" do
    test "returns error when no session found" do
      ...
      assert [%ActionEvent{} = note, %CompletedEvent{} = completed] = events
      assert note.action.kind == :warning
      assert note.action.detail.reason == :missing_session
      assert note.action.detail.thread_id == nil
      assert completed.ok == false
    ...
    test "returns error when session found without turn completion" do
      ...
      assert [%ActionEvent{} = note, %CompletedEvent{} = completed] = events
      assert note.action.detail.reason == :missing_turn_completion
      assert note.action.detail.thread_id == "t_1"
      assert completed.ok == false
    ...
    test "preserves resume token for future continuation" do
      ...
      {[_note, completed], _new_state} = CodexRunner.handle_stream_end(state)
    ...
    test "includes run context in terminal warning detail" do
      state = %{
        RunnerState.new(nil, nil, "agent:reviewer:main")
        | found_session: ResumeToken.new("codex", "thread_diag"),
          cwd: "/tmp/codex-project",
          run_id: "run_codex_789"
      }

      {[note, _completed], _new_state} = CodexRunner.handle_stream_end(state)

      assert note.action.detail.reason == :missing_turn_completion
      assert note.action.detail.thread_id == "thread_diag"
      assert note.action.detail.run_id == "run_codex_789"
      assert note.action.detail.session_key == "agent:reviewer:main"
      assert note.action.detail.cwd == "/tmp/codex-project"
      assert note.action.detail.has_partial_answer == false
    end
  end
...
defmodule AgentCore.CliRunners.CodexRunnerIntegrationTest do
  ...
  describe "GenServer lifecycle - start_link/1" do
    test "starts successfully with valid options" do
      ...
      {:ok, pid} =
        __MODULE__.MockCodexRunner.start_link(
          prompt: "test",
          cwd: System.tmp_dir!(),
          timeout: 10_000
        )
      ...
      # Wait for completion
      stream = __MODULE__.MockCodexRunner.stream(pid)
    ...
    test "handles missing prompt option" do
      ...
      result = __MODULE__.MockCodexRunner.start_link(cwd: "/tmp")
  ...
  describe "abort signal handling" do
    test "cancel stops running subprocess" do
      ...
      # Cancel the runner
      __MODULE__.MockCodexRunner.cancel(pid, :test_cancel)
  ...
  describe "session locking" do
    test "prevents concurrent runs of same session" do
      ...
      __MODULE__.MockCodexRunner.cancel(pid1)
