  ...
  describe "continue/3" do
    ...
    test "can override cwd" do
      ...
      override_cwd =
        Path.join(
          System.tmp_dir!(),
          "codex_subagent_override_#{System.unique_integer([:positive])}"
        )

      File.mkdir_p!(override_cwd)

      try do
        {:ok, new_session} = CodexSubagent.continue(session, "test", cwd: override_cwd)
        assert new_session.cwd == override_cwd
      catch
        :exit, _ -> :ok
      end
