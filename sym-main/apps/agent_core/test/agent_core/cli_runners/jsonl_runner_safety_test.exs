defmodule AgentCore.CliRunners.JsonlRunnerSafetyTest do
  ...
  test "concurrent new-session lock acquisition does not crash when the lock table is created lazily" do
    test_pid = self()

    # Remove the named ETS table so both runners race through the creation path.
    case :ets.whereis(AgentCore.CliRunners.JsonlRunner.SessionLocks) do
      :undefined -> :ok
      _ -> :ets.delete(AgentCore.CliRunners.JsonlRunner.SessionLocks)
    end
    ...
    start_runner = fn prompt ->
      Task.async(fn ->
        {:ok, runner_pid} =
          GenServer.start(
            AgentCore.CliRunners.JsonlRunner,
            {__MODULE__.ConcurrentSessionRunner,
             [prompt: prompt, cwd: System.tmp_dir!(), timeout: 5_000, owner: test_pid]}
          )

        stream = AgentCore.CliRunners.JsonlRunner.stream(runner_pid)
        {runner_pid, AgentCore.EventStream.events(stream) |> Enum.to_list()}
      end)
    end

    task_a = start_runner.("{\"type\":\"thread.started\",\"thread_id\":\"concurrent_lock_a\"}")
    task_b = start_runner.("{\"type\":\"thread.started\",\"thread_id\":\"concurrent_lock_b\"}")

    {_pid_a, events_a} = Task.await(task_a, 10_000)
    {_pid_b, events_b} = Task.await(task_b, 10_000)

    assert Enum.any?(events_a, &match?({:cli_event, %StartedEvent{}}, &1))
    assert Enum.any?(events_b, &match?({:cli_event, %StartedEvent{}}, &1))
  end

  defmodule ConcurrentSessionRunner do
    @behaviour AgentCore.CliRunners.JsonlRunner
    ...
    @impl true
    def build_command(prompt, _resume, _state) do
      {"bash", ["-c", "printf '%s\\n' '#{prompt}'"]}
    end
    ...
    @impl true
    def translate_event(%{"type" => "thread.started", "thread_id" => value}, state) do
      token = ResumeToken.new("codex", value)
