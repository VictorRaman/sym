defmodule AgentCore.LoopTest do
  ...
  describe "stream/4" do
    ...
    test "successful stream does not emit normal-path loop logs" do
      context = simple_context()
      response = Mocks.assistant_message("Quiet response")
      config = simple_config(stream_fn: Mocks.mock_stream_fn_single(response))
      ...
      refute log =~ "AgentCore.Loop starting"
      refute log =~ "AgentCore.Loop run_loop"
      refute log =~ "AgentCore.Loop do_inner_loop"
      refute log =~ "AgentCore.Loop turn completed"
      refute log =~ "AgentCore.Loop do_run_loop"
    end
  end
