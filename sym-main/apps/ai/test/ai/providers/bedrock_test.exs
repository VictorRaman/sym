...
defmodule Ai.Providers.BedrockStreamTest do
  ...
  describe "credential errors" do
    ...
    test "accepts credentials from opts.headers over env vars" do
      ...
      assert {:error, output} = result
      refute_missing_credentials(output)
    end
  ...
  defp refute_missing_credentials(%Ai.Types.AssistantMessage{error_message: error_message}) do
    refute error_message =~ "not found"
  end

  defp refute_missing_credentials(other) do
    refute other == :credential_error
  end
