...
Mesh mailbox runtime journal entries now carry `op_id`, `accepted_clock`, and `applied_clock`
metadata in addition to the user-visible prompt text. Runtime-injected `Ai.Types.UserMessage`
values now also persist internal `metadata` such as `op_id`, `envelope_id`, `handoff_id`, and
`accepted_clock`.
Runtime apply finalization is metadata-first: use `op_id` / `envelope_id` from the persisted user
message before falling back to legacy `text + timestamp` matching. Do not strip that metadata back
out when touching runtime mailbox code unless you also replace the apply-ledger contract end-to-end.
...
**Default `coding_tools/2`** (23 local-default tools registered in `CodingAgent.Tools.coding_tools/2` and `@builtin_tools` in `ToolRegistry`):
...
For channel-peer sessions, `exec`, `process`, and `await` are intentionally hidden from the default set so local sessions stay wider than remote/channel defaults.
