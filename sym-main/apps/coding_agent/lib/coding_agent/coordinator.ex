defmodule CodingAgent.Coordinator do
  ...
  def list_active(coordinator) do
    GenServer.call(coordinator, :list_active, 30_000)
  end
  ...
  def handle_call(:abort_all, _from, state) do
    ...
    {_results, final_state} = await_cleanup_completion(%{}, new_state)
  ...
  defp start_subagent(id, spec, state) do
    ...
    case final_prompt do
      ...
      prompt_text ->
        ...
        session_opts = build_session_opts(state, session_id, description)
  ...
  @spec build_session_opts(t(), String.t(), String.t()) :: keyword()
  defp build_session_opts(state, session_id, _description) do
    base_opts = [
      cwd: state.cwd,
      model: state.model,
  ...
  defp handle_await_message({:"$gen_call", from, :abort_all}, pending_ids, results, state) do
    ...
    new_state = abort_all_subagents(state)
    {final_results, final_state} = await_cleanup_completion(aborted_results, new_state)
    {:done, final_results, final_state}
  end
