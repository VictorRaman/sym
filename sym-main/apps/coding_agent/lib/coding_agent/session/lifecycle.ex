defmodule CodingAgent.Session.Lifecycle do
  ...
  def initialize(opts, session_pid) when is_list(opts) and is_pid(session_pid) do
    ...
    agent_id = normalize_agent_id(Keyword.get(opts, :agent_id, "default"))
    mesh_session_id = normalize_mesh_session_id(Keyword.get(opts, :mesh_session_id))
    mesh_mailbox_poll_interval_ms =
      normalize_poll_interval(Keyword.get(opts, :mesh_mailbox_poll_interval_ms))

    mesh_mailbox_lease_ms = normalize_lease_ms(Keyword.get(opts, :mesh_mailbox_lease_ms))
    ...
    tool_opts =
      ...
      |> Keyword.put(:mesh_session_id, mesh_session_id)
    ...
    state = %Session{
      ...
      mesh_mailbox_poll_interval_ms: mesh_mailbox_poll_interval_ms,
      mesh_mailbox_lease_ms: mesh_mailbox_lease_ms,
      mesh_mailbox_tick_ref: nil,
    ...
    {:ok, MeshMailboxRuntime.replay_pending_entries(state), lifecycle.extension_status_report}
  end
  ...
  def reset(state, reset_abort_wait_ms) when is_integer(reset_abort_wait_ms) do
    ...
    Notifier.ui_set_working_message(state, nil)
    state
    |> State.reset_runtime(new_session_manager, System.system_time(:millisecond))
    |> MeshMailboxRuntime.schedule_next()
  end
  ...
  defp normalize_agent_id(_agent_id), do: "default"

  defp normalize_mesh_session_id(mesh_session_id) when is_binary(mesh_session_id) do
    case String.trim(mesh_session_id) do
      "" -> nil
      trimmed -> trimmed
    end
  end

  defp normalize_mesh_session_id(_mesh_session_id), do: nil

  defp normalize_poll_interval(interval_ms) when is_integer(interval_ms) and interval_ms > 0,
    do: interval_ms

  defp normalize_poll_interval(_interval_ms), do: 2_000
