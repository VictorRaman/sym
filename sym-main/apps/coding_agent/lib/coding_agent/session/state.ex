defmodule CodingAgent.Session.State do
  ...
  def build_diagnostics(state) do
    ...
    %{
      ...
      mesh_session_id: Map.get(state, :mesh_session_id),
      mesh_mailbox_runtime_enabled: is_binary(Map.get(state, :mesh_session_id)),
      mesh_mailbox_journal_pending_count:
        RuntimeMailboxJournal.pending_count(state.session_manager.header.id),
      uptime_ms: uptime_ms,
