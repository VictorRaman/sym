defmodule CodingAgent.Tools.Task.Async do
  ...
  defp emit_task_started_event(task_id, run_id, lifecycle_context) do
    ...
    maybe_emit_mesh_task_event("task_started", payload, lifecycle_context)
  end
  ...
  defp emit_task_terminal_event(task_id, run_id, outcome, lifecycle_context) do
    ...
    maybe_emit_mesh_task_event(Atom.to_string(event_type), payload, lifecycle_context)
  end
  ...
  defp task_event_payload_base(task_id, run_id, lifecycle_context) do
    %{
      ...
      mesh_session_id: lifecycle_context[:mesh_session_id],
  ...
  defp maybe_emit_mesh_task_event(kind, payload, lifecycle_context) when is_binary(kind) do
    mesh_session_id = payload[:mesh_session_id] || lifecycle_context[:mesh_session_id]
    ...
    body =
      %{
        ...
        "status" => to_string(payload[:status] || kind),
        "ok" => payload[:ok],
    ...
    MeshBridge.append_task_event(mesh_session_id, kind, body,
      author: "coding_agent",
      scope: "mesh"
    )
  end
