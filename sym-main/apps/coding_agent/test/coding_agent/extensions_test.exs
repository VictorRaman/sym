    ...
    test "skips registration when provider already exists (built-in)", %{tmp_dir: tmp_dir} do
      ...
      report =
        capture_log([level: :debug], fn ->
          ...
          report = Extensions.register_extension_providers(extensions)
          send(self(), {:override_builtin_report, report})
        end)
        |> then(fn _log ->
          receive do
            {:override_builtin_report, report} -> report
          end
        end)
  ...
  describe "built-in provider shadowing scenarios" do
    ...
    test "built-in provider shadows single extension provider", %{tmp_dir: tmp_dir} do
      ...
      report =
        capture_log([level: :debug], fn ->
          ...
          report = Extensions.register_extension_providers(extensions)
          send(self(), {:shadowed_builtin_report, report})
        end)
        |> then(fn _log ->
          receive do
            {:shadowed_builtin_report, report} -> report
          end
        end)
    ...
    test "built-in provider shadows multiple extension providers and creates conflict", %{
      ...
      report =
        capture_log([level: :debug], fn ->
          report = Extensions.register_extension_providers(extensions)
          send(self(), {:builtin_shadow_report, report})
        end)
        |> then(fn _log ->
          receive do
            {:builtin_shadow_report, report} -> report
          end
        end)
    ...
    test "skips :storage type providers", %{tmp_dir: tmp_dir} do
      ...
      report =
        capture_log([level: :debug], fn ->
          ...
          report = Extensions.register_extension_providers(extensions)
          send(self(), {:storage_provider_report, report})
        end)
        |> then(fn _log ->
          receive do
            {:storage_provider_report, report} -> report
          end
        end)
    ...
    test "skips :tool_executor type providers", %{tmp_dir: tmp_dir} do
      ...
      report =
        capture_log([level: :debug], fn ->
          ...
          report = Extensions.register_extension_providers(extensions)
          send(self(), {:tool_executor_provider_report, report})
        end)
        |> then(fn _log ->
          receive do
            {:tool_executor_provider_report, report} -> report
          end
        end)
    ...
    test "mixes model and non-model providers correctly", %{tmp_dir: tmp_dir} do
      ...
      report =
        capture_log([level: :debug], fn ->
          ...
          report = Extensions.register_extension_providers(extensions)
          send(self(), {:mixed_type_provider_report, report})
        end)
        |> then(fn _log ->
          receive do
            {:mixed_type_provider_report, report} -> report
          end
        end)
    ...
    test "handles custom/unknown provider types", %{tmp_dir: tmp_dir} do
      ...
      report =
        capture_log([level: :debug], fn ->
          ...
          report = Extensions.register_extension_providers(extensions)
          send(self(), {:unknown_type_provider_report, report})
        end)
        |> then(fn _log ->
          receive do
            {:unknown_type_provider_report, report} -> report
          end
        end)
