defmodule CodingAgent.RateLimitRecoveryTest do
  ...
  describe "apply_strategy/2" do
    test "reset_backoff updates state" do
      ...
      {result, log} =
        capture_log([level: :info], fn ->
          result = RateLimitRecovery.apply_strategy(:reset_backoff, state)
          send(self(), {:reset_backoff_result, result})
        end)
        |> then(fn log ->
          receive do
            {:reset_backoff_result, result} -> {result, log}
          end
        end)
    ...
    test "fallback_model updates model in state" do
      ...
      {result, log} =
        capture_log([level: :info], fn ->
          result = RateLimitRecovery.apply_strategy({:fallback_model, @test_model}, state)
          send(self(), {:fallback_model_result, result})
        end)
        |> then(fn log ->
          receive do
            {:fallback_model_result, result} -> {result, log}
          end
        end)
    ...
    test "fallback_provider returns error for nonexistent provider" do
      ...
      {result, log} =
        capture_log([level: :info], fn ->
          ...
          send(self(), {:fallback_provider_result, result})
        end)
        |> then(fn log ->
          receive do
            {:fallback_provider_result, result} -> {result, log}
          end
        end)
    ...
    test "session_fork marks fork_requested" do
      ...
      {result, log} =
        capture_log([level: :info], fn ->
          result = RateLimitRecovery.apply_strategy({:session_fork, [preserve_messages: 5]}, state)
          send(self(), {:session_fork_result, result})
        end)
        |> then(fn log ->
          receive do
            {:session_fork_result, result} -> {result, log}
          end
        end)
    ...
    test "give_up returns error" do
      ...
      {result, log} =
        capture_log(fn ->
          ...
          send(self(), {:give_up_result, result})
        end)
        |> then(fn log ->
          receive do
            {:give_up_result, result} -> {result, log}
          end
        end)

      assert {:error, :recovery_strategies_exhausted} = result
      assert log =~ "All strategies exhausted"
    end
  end
