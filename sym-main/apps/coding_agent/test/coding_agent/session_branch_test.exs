defmodule CodingAgent.SessionBranchTest do
  ...
  describe "branch switching mid-operation" do
    ...
    test "branch switch after idle steering leaves no queue residue" do
      session = start_session()
      ...
      state_before = Session.get_state(session)
      assert :queue.len(state_before.steering_queue) == 0
      ...
      # Idle steering is consumed immediately, so navigation should not reintroduce queue residue.
      state_after = Session.get_state(session)
