defmodule CodingAgent.SessionEdgeCasesTest do
  ...
  defp wait_for_streaming_complete(session, timeout \\ 10_000) do
  ...
  defp do_wait_for_streaming_complete(session, deadline) do
    ...
    state = GenServer.call(session, :get_state, 15_000)
  ...
  defp wait_for_session_run(session, min_messages \\ 2, timeout \\ 5000) do
    deadline = System.monotonic_time(:millisecond) + timeout
    do_wait_for_session_run(session, min_messages, deadline)
  end

  defp do_wait_for_session_run(session, min_messages, deadline) do
    now = System.monotonic_time(:millisecond)

    if now >= deadline do
      raise "Timeout waiting for session run to complete"
    end

    state = Session.get_state(session)

    if not state.is_streaming and length(Session.get_messages(session)) >= min_messages do
      :ok
    else
      Process.sleep(10)
      do_wait_for_session_run(session, min_messages, deadline)
  ...
  defp wait_for_idle_and_empty_queues(session, timeout \\ 20_000) do
    deadline = System.monotonic_time(:millisecond) + timeout
    do_wait_for_idle_and_empty_queues(session, deadline)
  end

  defp do_wait_for_idle_and_empty_queues(session, deadline) do
    now = System.monotonic_time(:millisecond)

    if now >= deadline do
      raise "Timeout waiting for session queues to drain"
    end

    state = GenServer.call(session, :get_state, 15_000)

    if not state.is_streaming and
         :queue.len(state.steering_queue) == 0 and
         :queue.len(state.follow_up_queue) == 0 do
      :ok
    else
      Process.sleep(10)
      do_wait_for_idle_and_empty_queues(session, deadline)
  ...
  describe "steering and follow_up queue edge cases" do
    test "idle steering and follow_up calls do not leave queue residue" do
      session = start_session()
      ...
      wait_for_idle_and_empty_queues(session)
