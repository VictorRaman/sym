defmodule CodingAgent.SessionExtensionsTest do
  # This module dynamically defines extension modules and interacts with shared
  # extension/tool registry caches, so it must run synchronously.
  ...
  defp wait_for_streaming_complete(session, timeout \\ 20_000) do
  ...
  describe "extension status report" do
    ...
    test "get_extension_status_report/1 returns the status report", %{tmp_dir: tmp_dir} do
      ...
      # Verify the loaded extension is represented in the report.
      assert Enum.any?(report.extensions, fn ext ->
               ext.module == module and is_binary(ext.version)
             end)
    ...
    test "status report includes tool conflicts", %{tmp_dir: tmp_dir} do
      ...
      {report, log} =
        capture_log(fn ->
          session = start_session(cwd: tmp_dir)
          report = Session.get_extension_status_report(session)
          send(self(), {:tool_conflict_report, report})
        end)
        |> then(fn log ->
          receive do
            {:tool_conflict_report, report} -> {report, log}
          end
        end)
