defmodule CodingAgent.ToolRegistryTest do
  ...
  describe "get_tools/2" do
    ...
    test "hides local-only tools for channel peer sessions", %{tmp_dir: tmp_dir} do
      tools =
        ToolRegistry.get_tools(
          tmp_dir,
          session_key: "agent:test:telegram:default:dm:peer-123"
        )

      names = Enum.map(tools, & &1.name)

      refute "exec" in names
      refute "process" in names
      refute "await" in names
      assert "read" in names
      assert "bash" in names
    end
  end
  ...
  describe "wasm precedence and conflict reporting" do
    test "built-in tools win over wasm tools", %{tmp_dir: tmp_dir} do
      ...
      {tool, log} =
        capture_log(fn ->
          ...
          send(self(), {:builtin_wasm_tool, tool})
        end)
        |> then(fn log ->
          receive do
            {:builtin_wasm_tool, tool} -> {tool, log}
          end
        end)
    ...
    test "wasm tools shadow extension tools", %{tmp_dir: tmp_dir} do
      ...
      {tool, log} =
        capture_log(fn ->
          assert {:ok, tool} =
                   ToolRegistry.get_tool(tmp_dir, "wasm_shadow_test",
                     wasm_tools: wasm_inventory
                   )

          send(self(), {:wasm_shadow_tool, tool})
        end)
        |> then(fn log ->
          receive do
            {:wasm_shadow_tool, tool} -> {tool, log}
          end
        end)

      assert tool.label == "WASM Tool"
      assert log =~ "Tool name conflict"
      assert log =~ "wasm_shadow_test"
      assert log =~ "shadowed by wasm"
    ...
    test "tool_conflict_report includes wasm counts and sources", %{tmp_dir: tmp_dir} do
      ...
      {report, _log} =
        capture_log(fn ->
          ...
          send(self(), {:wasm_conflict_report, report})
        end)
        |> then(fn log ->
          receive do
            {:wasm_conflict_report, report} -> {report, log}
          end
        end)
