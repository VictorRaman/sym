defmodule CodingAgent.Tools.AgentTest do
  ...
  test "execute run async queues delegated run and poll returns completion" do
    ...
    assert result.details.mesh_session_id == "mesh_123"
    ...
    assert req.meta[:mesh_session_id] == "mesh_123"
  ...
  test "auto_followup falls back to router followup when session pid is unavailable" do
    ...
    assert followup.meta[:mesh_session_id] == "mesh_followup"
  end

  test "returns error when mesh_session_id is not a string" do
    assert {:error, "mesh_session_id must be a string"} =
             AgentTool.execute(
               "call_invalid_mesh",
               %{
                 "agent_id" => "oracle",
