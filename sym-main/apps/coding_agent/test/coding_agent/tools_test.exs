defmodule CodingAgent.ToolsTest do
  ...
  describe "coding_tools/2" do
    ...
    test "hides local-only tools for channel peer session_key" do
      tools =
        Tools.coding_tools(
          @test_cwd,
      ...
      refute "process" in tool_names
      refute "await" in tool_names
    end
  end
  ...
  describe "all_tools/2" do
    ...
    test "returns 24 tools (includes truncate plus local-only tools)" do
      tools_map = Tools.all_tools(@test_cwd)
  ...
  describe "specific tool definitions" do
    ...
    test "mesh_mailbox tool has expected structure" do
      tool = Tools.get_tool("mesh_mailbox", @test_cwd)

      assert %AgentTool{} = tool
      assert tool.name == "mesh_mailbox"
      assert Map.has_key?(tool.parameters["properties"], "action")
      assert Map.has_key?(tool.parameters["properties"], "mesh_session_id")
      assert Map.has_key?(tool.parameters["properties"], "message_id")
    end
