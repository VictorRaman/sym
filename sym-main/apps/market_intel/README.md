...
> **Status:** `incubator`
> **Default runtime:** no
> **Keep reason:** experimental market-data/commentary product line kept in-repo for iteration, but isolated from the default Lemon runtime

Shared umbrella defaults keep all ingestion/commentary workers disabled unless
explicitly enabled.
...
config :market_intel, :ingestion, %{
  enable_dex: false,         # DexScreener price ingestion
  enable_polymarket: false,  # Polymarket prediction markets
  enable_twitter: false,     # Twitter/X mention tracking
  enable_onchain: false,     # Base on-chain data
  enable_commentary: false,  # Commentary generation pipeline
  enable_scheduler: false    # Periodic commentary scheduling
}
```

Core infrastructure (`Cache` and `Repo`) always starts regardless of flags. In
the shared umbrella defaults, all of these flags stay disabled until an
