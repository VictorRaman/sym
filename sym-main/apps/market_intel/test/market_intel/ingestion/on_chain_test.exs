defmodule MarketIntel.Ingestion.OnChainTest do
  ...
  defp wait_for_state(pid, timeout_ms \\ 10_000) when is_pid(pid) do
    deadline = System.monotonic_time(:millisecond) + timeout_ms
    do_wait_for_state(pid, deadline)
  end

  defp do_wait_for_state(pid, deadline_ms) do
    try do
      :sys.get_state(pid, 200)
    catch
      :exit, _reason ->
        if System.monotonic_time(:millisecond) >= deadline_ms do
          flunk("timed out waiting for OnChain state")
        else
          Process.sleep(50)
          do_wait_for_state(pid, deadline_ms)
        end
    end
  end
end
