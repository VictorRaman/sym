defmodule LemonChannels.OutboxTest do
  ...
  describe "queue backpressure" do
    test "rejects enqueue when queue is full", %{outbox_pid: outbox_pid} do
      ...
      filled_queue =
        :queue.from_list([
          placeholder_entry("placeholder-1"),
          placeholder_entry("placeholder-2")
        ])
  ...
  defp placeholder_entry(id) do
    payload = %OutboundPayload{
      channel_id: "test-channel",
      kind: :text,
      content: id,
      account_id: "account-1",
      peer: %{kind: :dm, id: id, thread_id: nil}
    }
    ...
    %{
      ...
      available_at_ms: nil,
      chunk_index: 0,
      group_key: {payload.channel_id, payload.account_id, :dm, id, nil}
    }
  end
end
