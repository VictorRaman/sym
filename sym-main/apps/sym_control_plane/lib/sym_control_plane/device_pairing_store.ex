defmodule LemonControlPlane.DevicePairingStore do
  ...
  def put_device(device_token, value) when is_binary(device_token) and is_map(value),
    do: Store.put(@device_table, storage_key(device_token), value)

  @spec get_device(binary()) :: map() | nil
  def get_device(device_token) when is_binary(device_token),
    do: Store.get(@device_table, storage_key(device_token)) || Store.get(@device_table, device_token)
