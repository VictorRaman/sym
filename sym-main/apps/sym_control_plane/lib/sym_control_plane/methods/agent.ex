defmodule LemonControlPlane.Methods.Agent do
  ...
  defp validate_params(params) when is_map(params) do
    case get_param(params, "prompt") do
      nil ->
      ...
      prompt when is_binary(prompt) and byte_size(prompt) > 0 ->
        {:ok,
         %{
           ...
           queue_mode: QueueMode.parse(get_param(params, "queueMode"), default: :collect),
           cwd: get_param(params, "cwd"),
           tool_policy: get_param(params, "toolPolicy"),
           idempotency_key: get_param(params, "idempotencyKey")
         }}
  ...
  defp do_submit_run(params, session_key, ctx) do
    ...
    orchestrator = run_orchestrator_module()

    case Code.ensure_loaded(orchestrator) do
      {:module, _} ->
        submit_via_router(orchestrator, params, session_key, ctx)
  ...
  defp submit_via_router(orchestrator, params, session_key, ctx) do
    submit_params =
      RunRequest.new(%{
    ...
    case orchestrator.submit(submit_params) do
      {:ok, run_id} ->
        result = %{
          "run_id" => run_id,
          "session_key" => session_key
  ...
  defp run_orchestrator_module do
    Application.get_env(
      :lemon_control_plane,
