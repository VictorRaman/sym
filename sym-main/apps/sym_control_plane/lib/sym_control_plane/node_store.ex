defmodule LemonControlPlane.NodeStore do
  ...
  @pairing_display_code_table :nodes_pairing_display_codes
  ...
  def put_pairing_code(code, pairing_id) when is_binary(code) and is_binary(pairing_id),
    do: Store.put(@pairing_code_table, storage_key(code), pairing_id)

  @spec get_pairing_id_by_code(binary()) :: binary() | nil
  def get_pairing_id_by_code(code) when is_binary(code),
    do: Store.get(@pairing_code_table, storage_key(code)) || Store.get(@pairing_code_table, code)

  @spec put_pairing_display_code(binary(), binary()) :: :ok
  def put_pairing_display_code(pairing_id, code)
      when is_binary(pairing_id) and is_binary(code),
      do: Store.put(@pairing_display_code_table, pairing_id, code)

  @spec get_pairing_display_code(binary()) :: binary() | nil
  def get_pairing_display_code(pairing_id) when is_binary(pairing_id),
    do: Store.get(@pairing_display_code_table, pairing_id)

  @spec delete_pairing_display_code(binary()) :: :ok
  def delete_pairing_display_code(pairing_id) when is_binary(pairing_id),
    do: Store.delete(@pairing_display_code_table, pairing_id)
  ...
  def put_challenge(token, value) when is_binary(token) and is_map(value),
    do: Store.put(@challenge_table, storage_key(token), value)

  @spec get_challenge(binary()) :: map() | nil
  def get_challenge(token) when is_binary(token),
    do: Store.get(@challenge_table, storage_key(token)) || Store.get(@challenge_table, token)

  @spec delete_challenge(binary()) :: :ok
  def delete_challenge(token) when is_binary(token) do
    Store.delete(@challenge_table, storage_key(token))
    Store.delete(@challenge_table, token)
  end
  ...
  defp storage_key(token) when is_binary(token) do
