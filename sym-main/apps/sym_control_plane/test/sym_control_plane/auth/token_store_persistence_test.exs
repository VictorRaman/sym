defmodule LemonControlPlane.Auth.TokenStorePersistenceTest do
  ...
  describe "validate/1 with atom keys" do
    test "store/3 hashes the storage key and omits plaintext token from the stored record" do
      token = "hashed-store-token-#{System.unique_integer([:positive])}"
      identity = %{"type" => "device", "deviceId" => "dev-123"}

      assert {:ok, stored_info} = TokenStore.store(token, identity)

      hashed_key =
        :crypto.hash(:sha256, token)
        |> Base.encode16(case: :lower)

      refute Map.has_key?(stored_info, :token)
      assert LemonCore.Store.get(:session_tokens, token) == nil

      persisted = LemonCore.Store.get(:session_tokens, hashed_key)
      assert persisted != nil
      refute Map.has_key?(persisted, :token)
      refute Map.has_key?(persisted, "token")
      assert (persisted[:identity] || persisted["identity"]) == identity
    end
