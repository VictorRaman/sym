defmodule LemonControlPlane.Methods.NodePairStringKeysTest do
  ...
  describe "NodePairList with string keys" do
    test "lists pending pairings and restores code from the side table" do
      pairing_id = "list-pending-#{System.unique_integer([:positive])}"
      code = "LIST-#{System.unique_integer([:positive])}"

      LemonCore.Store.put(:nodes_pairing, pairing_id, %{
        "id" => pairing_id,
        "status" => "pending",
        "node_name" => "Listed Node",
        "node_type" => "agent",
        "capabilities" => %{"tools" => true},
        "expires_at_ms" => System.system_time(:millisecond) + 60_000,
        "created_at_ms" => System.system_time(:millisecond)
      ...
      LemonCore.Store.put(:nodes_pairing_by_code, code, pairing_id)
      LemonCore.Store.put(:nodes_pairing_display_codes, pairing_id, code)

      assert {:ok, %{"requests" => [request]}} = NodePairList.handle(%{}, @admin_ctx)
      assert request["pairingId"] == pairing_id
      assert request["code"] == code
      assert request["nodeName"] == "Listed Node"
    end
  end
end
