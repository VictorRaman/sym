defmodule LemonControlPlane.Protocol.SchemasTest do
  ...
  describe "validate/2" do
    ...
    test "validates mesh mailbox method schemas" do
      assert {:error, _} = Schemas.validate("mesh.agent.message", %{})

      assert :ok =
               Schemas.validate("mesh.agent.message", %{
                 "meshSessionId" => "mesh_123",
                 "agentId" => "reviewer",
                 "prompt" => "Review this"
               })
      ...
      assert :ok =
               Schemas.validate("mesh.agent.message", %{
                 "mesh_session_id" => "mesh_123",
                 "agent_id" => "reviewer",
                 "prompt" => "Review this"
               })

      assert {:error, _} = Schemas.validate("mesh.agent.mailbox.list", %{})

      assert :ok =
               Schemas.validate("mesh.agent.mailbox.list", %{
                 "meshSessionId" => "mesh_123",
                 "agentId" => "reviewer",
                 "limit" => 10,
                 "includeClaimed" => true
      ...
      assert {:error, _} = Schemas.validate("mesh.agent.mailbox.ack", %{})

      assert :ok =
               Schemas.validate("mesh.agent.mailbox.ack", %{
                 "meshSessionId" => "mesh_123",
                 "agentId" => "reviewer",
                 "messageId" => "msg_123",
                 "expectedClaimedBy" => "session:reviewer"
               })

      assert :ok = Schemas.validate("mesh.replication.status", %{})
      assert :ok = Schemas.validate("mesh.replication.status", nil)
    end
  end
end
