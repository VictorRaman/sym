defmodule LemonCore.Config do
  ...
  def reload(cwd \\ nil, opts \\ []) do
    base =
      if Code.ensure_loaded?(LemonCore.ConfigCache) and
           function_exported?(LemonCore.ConfigCache, :available?, 0) and
           LemonCore.ConfigCache.available?() do
        load_from_cache!(:reload, cwd, opts)
      else
        load_base_from_disk(cwd)
      end
  ...
  def load_file(path) do
    ...
    if File.exists?(expanded) do
      case TomlCompat.decode_file(expanded) do
        {:ok, map} ->
          stringify_keys(map)
  ...
  defp load_from_cache!(:get, cwd, opts) do
    try do
      LemonCore.ConfigCache.get(cwd, opts)
    catch
      :exit, reason -> reraise_validation_error_or_exit(reason)
    end
  end

  defp load_from_cache!(:reload, cwd, opts) do
    try do
  ...
  defp reraise_validation_error_or_exit(reason) do
    case extract_validation_error(reason) do
      %LemonCore.Config.ValidationError{} = error -> raise error
      nil -> exit(reason)
    end
  end

  defp extract_validation_error({{%LemonCore.Config.ValidationError{} = error, _stack}, _call}),
    do: error

  defp extract_validation_error({%LemonCore.Config.ValidationError{} = error, _stack}), do: error
  defp extract_validation_error(%LemonCore.Config.ValidationError{} = error), do: error
  defp extract_validation_error(_reason), do: nil
