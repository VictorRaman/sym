defmodule LemonCore.ConfigReloader.Watcher do
  ...
  def init(opts) do
    ...
    state = %{
      ...
      file_system_module: Keyword.get(opts, :file_system_module, FileSystem),
      watcher_pid: nil,
      debounce_ref: nil,
      mode: :polling
    }
  ...
  defp try_start_watcher(state) do
    file_system_module = state.file_system_module

    if Code.ensure_loaded?(file_system_module) do
      dirs = state.watch_targets

      case file_system_module.start_link(dirs: dirs) do
        {:ok, pid} ->
