defmodule LemonCore.Store do
  ...
  @doc """
  Atomically update a single key in a named table.

  The updater receives the current value (or `nil`) and must return one of:

  - `{:ok, new_value, reply}` to persist `new_value` and return `reply`
  - `{:delete, reply}` to delete the key and return `reply`
  - `{:reply, reply}` to return `reply` without mutating storage
  - `{:error, reason}` to abort and return `{:error, reason}`
  """
  @spec update(table :: atom(), key :: term(), (term() -> term())) :: term()
  def update(table, key, updater) when is_function(updater, 1) do
    safe_store_call({:generic_update, table, key, updater}, {:error, :store_unavailable},
      op: :update,
      table: table,
      key: key
    )
  ...
  def handle_call({:generic_update, table, key, updater}, _from, state) do
    case state.backend.get(state.backend_state, table, key) do
      {:ok, current_value, backend_state} ->
        state = %{state | backend_state: backend_state}

        case updater.(current_value) do
          {:ok, new_value, reply} ->
            case state.backend.put(state.backend_state, table, key, new_value) do
              {:ok, next_backend_state} ->
                if table in @generic_cached_tables do
                  ReadCache.put(table, key, new_value)
                end

                {:reply, reply, %{state | backend_state: next_backend_state}}

              {:error, reason} ->
                log_backend_error(:update_put, table, key, reason)
                {:reply, {:error, reason}, state}

              other ->
                log_backend_unexpected(:update_put, table, key, other)
                {:reply, {:error, {:unexpected_backend_response, other}}, state}
            end
          ...
          {:delete, reply} ->
            case state.backend.delete(state.backend_state, table, key) do
              ...
              {:error, reason} ->
                log_backend_error(:update_delete, table, key, reason)
                {:reply, {:error, reason}, state}

              other ->
                log_backend_unexpected(:update_delete, table, key, other)
                {:reply, {:error, {:unexpected_backend_response, other}}, state}
            end
          ...
          other ->
            {:reply, {:error, {:invalid_update_result, other}}, state}
        end

      {:error, reason} ->
        log_backend_error(:update_get, table, key, reason)
        {:reply, {:error, reason}, state}

      other ->
        log_backend_unexpected(:update_get, table, key, other)
        {:reply, {:error, {:unexpected_backend_response, other}}, state}
    end
  end
