defmodule Mix.Tasks.Lemon.QualityTest do
  ...
  describe "config validation" do
      ...
      output =
        capture_io(:stdio, fn ->
          capture_io(:stderr, fn ->
            try do
              Quality.run(["--validate-config", "--root", tmp_dir])
            rescue
              Mix.Error -> :ok
            end
          end)
    ...
    test "runs without --validate-config does not validate config" do
      tmp_dir = create_tmp_quality_repo()
      on_exit(fn -> File.rm_rf!(tmp_dir) end)
  ...
  describe "runtime noise" do
    test "does not start the umbrella runtime for docs-only checks" do
      tmp_dir = create_tmp_quality_repo()
      on_exit(fn -> File.rm_rf!(tmp_dir) end)
      ...
      newly_started = MapSet.difference(started_after, started_before)
      refute MapSet.member?(newly_started, :market_intel)
    end
  end
  ...
  describe "platform docs integration" do
    test "reports stale platform docs as a failing quality check" do
      tmp_dir = create_tmp_quality_repo()
      ...
      try do
        ...
        assert output =~ "[error] platform_docs check failed"
        assert output =~ "Platform tiers doc is stale"
      after
        File.rm_rf!(tmp_dir)
      end
    end
  ...
  defp create_tmp_quality_repo do
    ...
    File.write!(
      Path.join(tmp_dir, "docs/platform_tiers.md"),
      """
      # Platform Tiers
    ...
    File.write!(
      ...
      [
        ...
        %{path: "docs/platform_tiers.md", owner: "@test", last_reviewed: ~D[2026-03-10], max_age_days: 60}
      ]
      """
    )
