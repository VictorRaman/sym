defmodule LemonCore.ApplicationTest do
  ...
  defp restart_lemon_core! do
    _ = Application.stop(:lemon_core)
    assert {:ok, _} = Application.ensure_all_started(:lemon_core)
  end

  defp wait_for_pid(name, previous_pid, timeout_ms \\ 5_000) do
    deadline = System.monotonic_time(:millisecond) + timeout_ms
    do_wait_for_pid(name, previous_pid, deadline)
  end

  defp do_wait_for_pid(name, previous_pid, deadline_ms) do
    case Process.whereis(name) do
      pid when is_pid(pid) and pid != previous_pid ->
        if Process.alive?(pid) do
          pid
        else
          maybe_retry_wait(name, previous_pid, deadline_ms)
        end
  ...
  defp maybe_retry_wait(name, previous_pid, deadline_ms) do
    if System.monotonic_time(:millisecond) >= deadline_ms do
      nil
    else
      Process.sleep(50)
      do_wait_for_pid(name, previous_pid, deadline_ms)
    end
  end
