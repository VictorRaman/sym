defmodule LemonGateway.Engines.CliAdapter do
  ...
  defp start_runner(runner_module, engine_id, job, opts) do
    ...
    start_opts =
      ...
      |> maybe_put(:mesh_session_id, get_in(job.meta || %{}, [:mesh_session_id]))
      |> maybe_put(:model, runner_model_override(engine_id, get_in(job.meta || %{}, [:model])))
  ...
  defp runner_model_override("codex", model) when is_binary(model) do
    ...
    cond do
      ...
      String.starts_with?(trimmed, "codex:") ->
        trimmed

      String.starts_with?(trimmed, "openai-codex:") ->
        trimmed

      String.starts_with?(trimmed, "openai:") ->
        trimmed

      String.starts_with?(trimmed, "chatgpt:") ->
        trimmed

      String.starts_with?(trimmed, "gpt-") ->
        trimmed

      String.starts_with?(trimmed, "o1") or
          String.starts_with?(trimmed, "o3") or
          String.starts_with?(trimmed, "o4") ->
        trimmed
  ...
  defp runner_model_override(_engine_id, model), do: model
