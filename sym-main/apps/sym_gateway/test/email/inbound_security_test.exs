defmodule LemonGateway.EmailInboundSecurityTest do
  ...
  defmodule EmailInboundTestRunOrchestrator do
    def submit(%RunRequest{run_id: run_id}) when is_binary(run_id), do: {:ok, run_id}
    def submit(%RunRequest{}), do: {:ok, LemonCore.Id.run_id()}
  end
  ...
  setup do
    ...
    Application.put_env(:lemon_core, :router_bridge, %{run_orchestrator: EmailInboundTestRunOrchestrator})
    ...
    on_exit(fn ->
      ...
      clear_table(@message_thread_table)
      clear_table(@thread_state_table)
      Application.put_env(:lemon_core, :router_bridge, original_bridge)
    end)
