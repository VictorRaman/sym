defmodule LemonRouter.SessionCoordinatorTest do
  ...
  test "query helpers expose router-owned active session state", %{run_supervisor: run_supervisor} do
    ...
    refute Enum.any?(SessionCoordinator.list_active_sessions(), &(&1.session_key == session_key))
    ...
    assert eventually(fn ->
             not Enum.any?(SessionCoordinator.list_active_sessions(), &(&1.session_key == session_key))
           end)
  end
