defmodule LemonSkills.Config do
  ...
  defp compat_global_skills_dir do
    resolved_agent_dir = Path.expand(agent_dir())
    agent_parent = Path.dirname(resolved_agent_dir)

    compat_root =
      if Path.basename(resolved_agent_dir) == "agent" and Path.basename(agent_parent) == ".lemon" do
        Path.dirname(agent_parent)
      else
        agent_parent
      end

    Path.join([compat_root, ".agents", "skills"])
  end
