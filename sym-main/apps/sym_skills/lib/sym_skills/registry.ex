defmodule LemonSkills.Registry do
  ...
  @global_skills_cache_key {__MODULE__, :global_skills}
  @project_skills_cache_table :lemon_skills_project_cache
  ...
  def find_relevant(context, opts \\ []) when is_binary(context) do
    normalized_context = String.trim(context)
    cwd = Keyword.get(opts, :cwd)
    ...
    if normalized_context == "" do
      []
    else
      context_lower = String.downcase(normalized_context)
  ...
  @impl true
  def init(_opts) do
    ensure_project_skills_cache_table()
  ...
  defp load_global_skills(state) do
    skills = load_global_skills_map()
    cache_global_skills(skills)

    %{state | global_skills: skills}
  end

  defp load_project_skills(state, cwd) when is_binary(cwd) do
    skills = load_project_skills_map(cwd)
    cache_project_skills(cwd, skills)
      ...
      {:ok, content} ->
        case Manifest.parse(content) do
          {:ok, manifest, body} ->
            ...
            |> Entry.with_body_search_text(normalize_body_search_text(body))
  ...
  defp load_global_skills_map do
  ...
  defp load_project_skills_map(cwd) when is_binary(cwd) do
  ...
  defp calculate_relevance(%Entry{} = entry, context_lower, context_words) do
    ...
    body_lower = entry.body_search_text || ""
  ...
  defp normalize_body_search_text(body) when is_binary(body) do
    body
    |> String.slice(0, 10_000)
    |> String.downcase()
  end

  defp normalize_body_search_text(_body), do: ""
  ...
  defp merged_skills_snapshot(nil), do: global_skills_snapshot()

  defp merged_skills_snapshot(cwd) when is_binary(cwd) do
    Map.merge(global_skills_snapshot(), project_skills_snapshot(cwd))
  end

  defp global_skills_snapshot do
    :persistent_term.get(@global_skills_cache_key, %{})
  end

  defp project_skills_snapshot(cwd) when is_binary(cwd) do
    ensure_project_skills_cache_table()

    case :ets.lookup(@project_skills_cache_table, cwd) do
      [{^cwd, skills}] ->
        skills
  ...
  defp cache_state(state) do
    cache_global_skills(state.global_skills)
    ensure_project_skills_cache_table()

    Enum.each(state.project_skills, fn {cwd, skills} ->
      cache_project_skills(cwd, skills)
    end)
  ...
  defp cache_global_skills(skills) when is_map(skills) do
    :persistent_term.put(@global_skills_cache_key, skills)
  end

  defp cache_project_skills(cwd, skills) when is_binary(cwd) and is_map(skills) do
    ensure_project_skills_cache_table()
    :ets.insert(@project_skills_cache_table, {cwd, skills})
  end

  defp ensure_project_skills_cache_table do
    case :ets.whereis(@project_skills_cache_table) do
      :undefined ->
        try do
          :ets.new(@project_skills_cache_table, [
            :named_table,
            :public,
            :set,
