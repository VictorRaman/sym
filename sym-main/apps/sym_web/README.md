...
**Behavior:**
1. If `config :lemon_web, :access_token` is `nil` or `""`, all requests pass through (no gate). When the endpoint is bound to `0.0.0.0`, Lemon logs a warning at startup so this fail-open deployment is visible.
