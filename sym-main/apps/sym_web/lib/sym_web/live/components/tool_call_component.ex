defmodule LemonWeb.Live.Components.ToolCallComponent do
  ...
  defp format_payload(value) when is_binary(value), do: Redaction.redact_term(value)

  defp format_payload(value) when is_map(value) or is_list(value) do
    redacted = Redaction.redact_term(value)

    case Jason.encode(redacted, pretty: true) do
      {:ok, encoded} ->
        encoded

      {:error, _reason} ->
        inspect(redacted, pretty: true, printable_limit: :infinity)
    end
  end

  defp format_payload(value),
    do: inspect(Redaction.redact_term(value), pretty: true, printable_limit: :infinity)
end
