...
The `native/lemon-wasm-runtime/` directory only needs source files (`src/`, `wit/`, `Cargo.toml`,
`Cargo.lock`). A local `native/lemon-wasm-runtime/target/` directory is a disposable Rust build
artifact, not part of the checked-in runtime layout, and can be safely removed or reported by
`mix lemon.cleanup`.
